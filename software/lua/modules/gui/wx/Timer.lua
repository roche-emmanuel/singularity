...
function Class:initialize(options)
	...
	--[[for name,func in pairs(wx.wxTimer) do
		if(type(func)=="function" and not self[name]) and name~="new" and name~="__eq" and name~="__gc" then
			...
			local wname = (name:sub(1,5)=="base_" and name) or (wx.wxTimer["base_"..name] and "base_"..name) or name -- force rediction to the base function call to avoid infinite looping.
