local Class = createClass{name="BasicView",bases="core.Object"};
...
function Class:initialize(options)
	self:check(options,"Invalid options for basic view creation.")
	self:check(options.view,"Invalid awesomium webview")
	self:check(options.surfaceFactory,"Invalid surface factory")
	
	self._surfaceFactory = options.surfaceFactory
...
function Class:createSurface(options)
	-- create the surface for this webview:
	-- self:showMessage("Creating web view surface...")
	self._surface = self._surfaceFactory:getOrCreateSurface(self._view,options)
	self:check(self._surface,"Invalid WebView surface object.")
	-- self:showMessage("Webview surface created.")
end

function Class:isValid()
	return self._surface:isValid()
end

function Class:destroy()
	self:info("Destroying basic web view with ID: ",self._id)
	if self._view then
		self._view:Destroy()
		-- self:getManager():update()
		self._view = nil
	end
	
	if self._surface then
		self._surfaceFactory:destroySurface(self._surface)
		self._surface = nil
	end	
end
...
function Class:setupWebView(options)
	...
	self._view:SetTransparent(options.transparent or false)
...
function Class:loadURL(url)
	...
	self._view:LoadURL(awe.WebURL(url))
end
...
function Class:checkErrors(obj)
	local err = (obj or self._view):last_error()
...
function Class:reload(ignore_cache)
	...
	self._view:Reload(ignore_cache==nil or ignore_cache)
	-- self._view:Reload(true)
	-- self:loadURL(self._view:url():spec())
end
...
function Class:setupJSHandler()
	...
	self._view:set_js_method_handler(self._jsHandler)
end
...
function Class:setupLoadListener()
	...
	self._view:set_load_listener(self._loadListener)
end
...
function Class:setupProcessListener()
	...
	self._view:set_process_listener(self._processListener)
end
...
function Class:setupViewListener()
	...
	self._view:set_view_listener(self._viewListener)
end
...
function Class:injectKeyboardEvent(ke)
	self._view:InjectKeyboardEvent(ke)
end

function Class:injectMouseMove(x,y)
	self._view:InjectMouseMove(x,y)
end

function Class:injectMouseDown(mb)
	self._view:InjectMouseDown(mb)
end

function Class:injectMouseUp(mb)
	self._view:InjectMouseUp(mb)
end

function Class:injectMouseWheel(vwm,hwm)
	self._view:InjectMouseWheel(vwm,hwm)
end

function Class:executeJavascript(script,frame_xpath)
	self._view:ExecuteJavascript(script,frame_xpath or "")
end

function Class:executeJavascriptWithResult(script,frame_xpath)
	return self._view:ExecuteJavascriptWithResult(script,frame_xpath or "")
end
