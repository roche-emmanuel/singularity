#ifndef _WRAPPERS_WRAPPER_BTQUANTIZEDBVH_H_
#define _WRAPPERS_WRAPPER_BTQUANTIZEDBVH_H_
...
class wrapper_btQuantizedBvh : public btQuantizedBvh, public luna_wrapper_base {

public:
		

	~wrapper_btQuantizedBvh() {
		logDEBUG3("Calling delete function for wrapper btQuantizedBvh");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((btQuantizedBvh*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_btQuantizedBvh(lua_State* L, lua_Table* dum) 
		: btQuantizedBvh(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((btQuantizedBvh*)this);
			_obj.callFunction<void>();
		}
	};
	...
	int calculateSerializeBufferSizeNew() const {
		...
		return btQuantizedBvh::calculateSerializeBufferSizeNew();
	};
	...
	static int _bind_public_setInternalNodeAabbMin(lua_State *L) {
		...
		wrapper_btQuantizedBvh* self=Luna< btQuantizedBvh >::checkSubType< wrapper_btQuantizedBvh >(L,1);
		if(!self) {
			luna_printStack(L);
