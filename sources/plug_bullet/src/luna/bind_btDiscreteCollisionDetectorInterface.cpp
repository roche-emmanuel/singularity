...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(btDiscreteCollisionDetectorInterface*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_getClosestPoints(lua_State *L) {
		...
		btDiscreteCollisionDetectorInterface* self=(Luna< btDiscreteCollisionDetectorInterface >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void btDiscreteCollisionDetectorInterface::getClosestPoints(const btDiscreteCollisionDetectorInterface::ClosestPointInput &, btDiscreteCollisionDetectorInterface::Result &, class btIDebugDraw *, bool). Got : '%s'\n%s",typeid(Luna< btDiscreteCollisionDetectorInterface >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
