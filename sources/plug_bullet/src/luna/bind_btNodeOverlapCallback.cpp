...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(btNodeOverlapCallback*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_processNode(lua_State *L) {
		...
		btNodeOverlapCallback* self=(Luna< btNodeOverlapCallback >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void btNodeOverlapCallback::processNode(int, int). Got : '%s'\n%s",typeid(Luna< btNodeOverlapCallback >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
