...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(btVehicleRaycaster*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_castRay(lua_State *L) {
		...
		btVehicleRaycaster* self=(Luna< btVehicleRaycaster >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void * btVehicleRaycaster::castRay(const btVector3 &, const btVector3 &, btVehicleRaycaster::btVehicleRaycasterResult &). Got : '%s'\n%s",typeid(Luna< btVehicleRaycaster >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
