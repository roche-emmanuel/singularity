...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(btOverlapCallback*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_processOverlap(lua_State *L) {
		...
		btOverlapCallback* self=(Luna< btOverlapCallback >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool btOverlapCallback::processOverlap(btBroadphasePair &). Got : '%s'\n%s",typeid(Luna< btOverlapCallback >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
