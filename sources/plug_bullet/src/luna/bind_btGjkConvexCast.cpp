...
public:
	...
	static int _bind_calcTimeOfImpact(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call bool btGjkConvexCast::calcTimeOfImpact(const btTransform &, const btTransform &, const btTransform &, const btTransform &, btConvexCast::CastResult &). Got : '%s'\n%s",typeid(Luna< btConvexCast >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_base_calcTimeOfImpact(lua_State *L) {
		...
		btGjkConvexCast* self=Luna< btConvexCast >::checkSubType< btGjkConvexCast >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call bool btGjkConvexCast::base_calcTimeOfImpact(const btTransform &, const btTransform &, const btTransform &, const btTransform &, btConvexCast::CastResult &). Got : '%s'\n%s",typeid(Luna< btConvexCast >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
