...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(btBroadphaseAabbCallback*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		btBroadphaseAabbCallback* self= (btBroadphaseAabbCallback*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< btBroadphaseAabbCallback >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_process(lua_State *L) {
		...
		btBroadphaseAabbCallback* self=(Luna< btBroadphaseAabbCallback >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool btBroadphaseAabbCallback::process(const btBroadphaseProxy *). Got : '%s'\n%s",typeid(Luna< btBroadphaseAabbCallback >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
...
luna_RegType LunaTraits< btBroadphaseAabbCallback >::methods[] = {
	...
	{"fromVoid", &luna_wrapper_btBroadphaseAabbCallback::_bind_fromVoid},
	{"asVoid", &luna_wrapper_btBroadphaseAabbCallback::_bind_asVoid},
