...
public:
	...
	static int _bind_contents(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IDocIterator * IDocEntry::contents() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocEntry >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocEntry::_bind___eq},
