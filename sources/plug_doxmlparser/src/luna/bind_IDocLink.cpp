...
public:
	...
	static int _bind_refId(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocLink::refId() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_text(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocLink::text() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocLink >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocLink::_bind___eq},
