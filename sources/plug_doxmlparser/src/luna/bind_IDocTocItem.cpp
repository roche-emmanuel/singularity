...
public:
	...
	static int _bind_id(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocTocItem::id() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_title(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocTocItem::title() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocTocItem >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocTocItem::_bind___eq},
