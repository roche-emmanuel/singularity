...
public:
	...
	static int _bind_baseCompounds(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IRelatedCompoundIterator * IInterface::baseCompounds() const. Got : '%s'",typeid(Luna< ICompound >::check(L,1)).name());
		}
	...
	static int _bind_derivedCompounds(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IRelatedCompoundIterator * IInterface::derivedCompounds() const. Got : '%s'",typeid(Luna< ICompound >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IInterface >::methods[] = {
	...
	{"__eq", &luna_wrapper_IInterface::_bind___eq},
