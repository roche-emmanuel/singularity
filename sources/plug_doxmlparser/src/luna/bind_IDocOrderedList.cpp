...
public:
	...
	static int _bind_elements(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IDocIterator * IDocOrderedList::elements() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocOrderedList >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocOrderedList::_bind___eq},
