...
public:
	...
	static int _bind_contents(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IDocIterator * IDocRoot::contents() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_internal(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IDocInternal * IDocRoot::internal() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocRoot >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocRoot::_bind___eq},
