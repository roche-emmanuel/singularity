...
public:
	...
	static int _bind_text(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * ILT_Text::text() const. Got : '%s'",typeid(Luna< ILinkedText >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< ILT_Text >::methods[] = {
	...
	{"__eq", &luna_wrapper_ILT_Text::_bind___eq},
