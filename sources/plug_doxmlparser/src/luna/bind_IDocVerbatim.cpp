...
public:
	...
	static int _bind_text(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocVerbatim::text() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_type(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IDocVerbatim::Types IDocVerbatim::type() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocVerbatim >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocVerbatim::_bind___eq},
