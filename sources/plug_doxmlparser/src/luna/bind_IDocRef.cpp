...
public:
	...
	static int _bind_refId(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocRef::refId() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_targetKind(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call IDocRef::TargetKind IDocRef::targetKind() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_external(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocRef::external() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
	...
	static int _bind_text(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const IString * IDocRef::text() const. Got : '%s'",typeid(Luna< IDoc >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< IDocRef >::methods[] = {
	...
	{"__eq", &luna_wrapper_IDocRef::_bind___eq},
