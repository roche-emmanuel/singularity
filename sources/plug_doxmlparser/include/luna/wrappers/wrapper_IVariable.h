#ifndef _WRAPPERS_WRAPPER_IVARIABLE_H_
#define _WRAPPERS_WRAPPER_IVARIABLE_H_
...
class wrapper_IVariable : public IVariable, public luna_wrapper_base {

public:
		

	~wrapper_IVariable() {
		logDEBUG3("Calling delete function for wrapper IVariable");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IVariable*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IVariable(lua_State* L, lua_Table* dum) 
		: IVariable(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IVariable*)this);
			_obj.callFunction<void>();
		}
	};
