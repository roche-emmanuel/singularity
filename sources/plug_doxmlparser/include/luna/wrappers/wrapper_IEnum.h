#ifndef _WRAPPERS_WRAPPER_IENUM_H_
#define _WRAPPERS_WRAPPER_IENUM_H_
...
class wrapper_IEnum : public IEnum, public luna_wrapper_base {

public:
		

	~wrapper_IEnum() {
		logDEBUG3("Calling delete function for wrapper IEnum");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IEnum*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IEnum(lua_State* L, lua_Table* dum) 
		: IEnum(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IEnum*)this);
			_obj.callFunction<void>();
		}
	};
	...
	IMemberIterator * enumValues() const {
		THROW_IF(!_obj.pushFunction("enumValues"),"No implementation for abstract function IEnum::enumValues");
		_obj.pushArg((IEnum*)this);
