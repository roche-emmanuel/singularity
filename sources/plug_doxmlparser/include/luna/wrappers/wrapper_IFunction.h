#ifndef _WRAPPERS_WRAPPER_IFUNCTION_H_
#define _WRAPPERS_WRAPPER_IFUNCTION_H_
...
class wrapper_IFunction : public IFunction, public luna_wrapper_base {

public:
		

	~wrapper_IFunction() {
		logDEBUG3("Calling delete function for wrapper IFunction");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IFunction*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IFunction(lua_State* L, lua_Table* dum) 
		: IFunction(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IFunction*)this);
			_obj.callFunction<void>();
		}
	};
