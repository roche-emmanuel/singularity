#ifndef _WRAPPERS_WRAPPER_ISLOT_H_
#define _WRAPPERS_WRAPPER_ISLOT_H_
...
class wrapper_ISlot : public ISlot, public luna_wrapper_base {

public:
		

	~wrapper_ISlot() {
		logDEBUG3("Calling delete function for wrapper ISlot");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((ISlot*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_ISlot(lua_State* L, lua_Table* dum) 
		: ISlot(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((ISlot*)this);
			_obj.callFunction<void>();
		}
	};
