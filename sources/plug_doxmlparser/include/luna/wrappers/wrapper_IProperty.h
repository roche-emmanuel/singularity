#ifndef _WRAPPERS_WRAPPER_IPROPERTY_H_
#define _WRAPPERS_WRAPPER_IPROPERTY_H_
...
class wrapper_IProperty : public IProperty, public luna_wrapper_base {

public:
		

	~wrapper_IProperty() {
		logDEBUG3("Calling delete function for wrapper IProperty");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IProperty*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IProperty(lua_State* L, lua_Table* dum) 
		: IProperty(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IProperty*)this);
			_obj.callFunction<void>();
		}
	};
