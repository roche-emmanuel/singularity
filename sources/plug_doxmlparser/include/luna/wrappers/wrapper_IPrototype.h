#ifndef _WRAPPERS_WRAPPER_IPROTOTYPE_H_
#define _WRAPPERS_WRAPPER_IPROTOTYPE_H_
...
class wrapper_IPrototype : public IPrototype, public luna_wrapper_base {

public:
		

	~wrapper_IPrototype() {
		logDEBUG3("Calling delete function for wrapper IPrototype");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IPrototype*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IPrototype(lua_State* L, lua_Table* dum) 
		: IPrototype(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IPrototype*)this);
			_obj.callFunction<void>();
		}
	};
