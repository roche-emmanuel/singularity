#ifndef _WRAPPERS_WRAPPER_ISIGNAL_H_
#define _WRAPPERS_WRAPPER_ISIGNAL_H_
...
class wrapper_ISignal : public ISignal, public luna_wrapper_base {

public:
		

	~wrapper_ISignal() {
		logDEBUG3("Calling delete function for wrapper ISignal");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((ISignal*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_ISignal(lua_State* L, lua_Table* dum) 
		: ISignal(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((ISignal*)this);
			_obj.callFunction<void>();
		}
	};
