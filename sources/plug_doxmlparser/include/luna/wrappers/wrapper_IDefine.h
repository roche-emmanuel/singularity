#ifndef _WRAPPERS_WRAPPER_IDEFINE_H_
#define _WRAPPERS_WRAPPER_IDEFINE_H_
...
class wrapper_IDefine : public IDefine, public luna_wrapper_base {

public:
		

	~wrapper_IDefine() {
		logDEBUG3("Calling delete function for wrapper IDefine");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IDefine*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IDefine(lua_State* L, lua_Table* dum) 
		: IDefine(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IDefine*)this);
			_obj.callFunction<void>();
		}
	};
