#ifndef _WRAPPERS_WRAPPER_IEXCEPTION_H_
#define _WRAPPERS_WRAPPER_IEXCEPTION_H_
...
class wrapper_IException : public IException, public luna_wrapper_base {

public:
		

	~wrapper_IException() {
		logDEBUG3("Calling delete function for wrapper IException");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IException*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IException(lua_State* L, lua_Table* dum) 
		: IException(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IException*)this);
			_obj.callFunction<void>();
		}
	};
