#ifndef _WRAPPERS_WRAPPER_IMEMBER_H_
#define _WRAPPERS_WRAPPER_IMEMBER_H_
...
class wrapper_IMember : public IMember, public luna_wrapper_base {

public:
		

	~wrapper_IMember() {
		logDEBUG3("Calling delete function for wrapper IMember");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IMember*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IMember(lua_State* L, lua_Table* dum) 
		: IMember(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IMember*)this);
			_obj.callFunction<void>();
		}
	};
