#ifndef _WRAPPERS_WRAPPER_IFRIEND_H_
#define _WRAPPERS_WRAPPER_IFRIEND_H_
...
class wrapper_IFriend : public IFriend, public luna_wrapper_base {

public:
		

	~wrapper_IFriend() {
		logDEBUG3("Calling delete function for wrapper IFriend");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IFriend*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IFriend(lua_State* L, lua_Table* dum) 
		: IFriend(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IFriend*)this);
			_obj.callFunction<void>();
		}
	};
