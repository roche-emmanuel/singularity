#ifndef _WRAPPERS_WRAPPER_IDCOP_H_
#define _WRAPPERS_WRAPPER_IDCOP_H_
...
class wrapper_IDCOP : public IDCOP, public luna_wrapper_base {

public:
		

	~wrapper_IDCOP() {
		logDEBUG3("Calling delete function for wrapper IDCOP");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IDCOP*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IDCOP(lua_State* L, lua_Table* dum) 
		: IDCOP(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IDCOP*)this);
			_obj.callFunction<void>();
		}
	};
