#ifndef _WRAPPERS_WRAPPER_IENUMVALUE_H_
#define _WRAPPERS_WRAPPER_IENUMVALUE_H_
...
class wrapper_IEnumValue : public IEnumValue, public luna_wrapper_base {

public:
		

	~wrapper_IEnumValue() {
		logDEBUG3("Calling delete function for wrapper IEnumValue");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((IEnumValue*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_IEnumValue(lua_State* L, lua_Table* dum) 
		: IEnumValue(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((IEnumValue*)this);
			_obj.callFunction<void>();
		}
	};
	...
	const IString * name() const {
		THROW_IF(!_obj.pushFunction("name"),"No implementation for abstract function IEnumValue::name");
		_obj.pushArg((IEnumValue*)this);
		return (_obj.callFunction<IString*>());
	};
