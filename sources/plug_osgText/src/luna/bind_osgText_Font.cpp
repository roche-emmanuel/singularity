...
public:
	...
	inline static bool _lg_typecheck_getGlyph(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,2,48492479) ) return false;
		if( (lua_isnumber(L,3)==0 || lua_tointeger(L,3) != lua_tonumber(L,3)) ) return false;
		return true;
	}
	...
	// void osgText::Font::resizeGLObjectBuffers(unsigned int maxSize)
	static int _bind_resizeGLObjectBuffers(lua_State *L) {
		if (!_lg_typecheck_resizeGLObjectBuffers(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in void osgText::Font::resizeGLObjectBuffers(unsigned int maxSize) function, expected prototype:\nvoid osgText::Font::resizeGLObjectBuffers(unsigned int maxSize)\nClass arguments details:\n");
		}
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgText::Font::resizeGLObjectBuffers(unsigned int)");
		}
...
luna_RegType LunaTraits< osgText::Font >::methods[] = {
	...
	{"resizeGLObjectBuffers", &luna_wrapper_osgText_Font::_bind_resizeGLObjectBuffers},
