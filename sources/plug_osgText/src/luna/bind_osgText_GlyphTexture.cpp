...
public:
	...
	// void osgText::GlyphTexture::resizeGLObjectBuffers(unsigned int maxSize)
	static int _bind_resizeGLObjectBuffers(lua_State *L) {
		if (!_lg_typecheck_resizeGLObjectBuffers(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in void osgText::GlyphTexture::resizeGLObjectBuffers(unsigned int maxSize) function, expected prototype:\nvoid osgText::GlyphTexture::resizeGLObjectBuffers(unsigned int maxSize)\nClass arguments details:\n");
		}
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgText::GlyphTexture::resizeGLObjectBuffers(unsigned int)");
		}
...
luna_RegType LunaTraits< osgText::GlyphTexture >::methods[] = {
	...
	{"resizeGLObjectBuffers", &luna_wrapper_osgText_GlyphTexture::_bind_resizeGLObjectBuffers},
