...
TaskGraph::TaskGraph(Task* task) : Task("TaskGraph", false, 0)
{
    addTask(task);
}
...
void TaskGraph::init(TaskSet& initialized)
{
...
TaskGraph::TaskIterator TaskGraph::getDependencies(Task* t)
{
    TaskDependencyMap::iterator i;
...
TaskGraph::TaskIterator TaskGraph::getInverseDependencies(Task* t)
{
    TaskDependencyMap::iterator i;
...
{
    CHECK(sgtCast(t,TaskGraph) == NULL || !sgtCast(t,TaskGraph)->isEmpty(),"Invalid task argument.");
...
void TaskGraph::removeTask(Task* t)
{
    TaskSet::iterator i = allTasks.find(t);
    if (i != allTasks.end()) {
        ...
        firstTasks.erase(t);
        lastTasks.erase(t);
        CHECK(dependencies.find(t) == dependencies.end(),"Found invalid task in dependency map");
        CHECK(inverseDependencies.find(t) == inverseDependencies.end(),"Found invalid task in inverse dependency map");
    }
}

void TaskGraph::addDependency(Task* src, Task* dst)
{
...
void TaskGraph::removeDependency(Task* src, Task* dst)
{
    assert(allTasks.find(src) != allTasks.end());
    assert(allTasks.find(dst) != allTasks.end());
    // updates the successors and predecessors maps
...
void TaskGraph::removeAndGetDependencies(Task* src, TaskSet& deletedDependencies)
{
    // find the set of dependencies for this task
    TaskDependencyMap::iterator it = dependencies.find(src);
    if (it != dependencies.end()) { // there are dependencies
        TaskSet dests = it->second;
        for (TaskSet::iterator i = dests.begin(); i != dests.end(); ++i) {
            deletedDependencies.insert(*i);
            TaskSet dstSet = inverseDependencies[*i];
...
void TaskGraph::taskStateChanged(Task* t, bool done, reason r)
{
    ...
    if (!done) {
        if (r != DATA_NEEDED) {
            ...
            while (i.hasNext()) {
                i.next()->setIsDone(false, 0, DEPENDENCY_CHANGED);
            }
        }
    } else {
...
void TaskGraph::completionDateChanged(Task* t, unsigned int date)
{
