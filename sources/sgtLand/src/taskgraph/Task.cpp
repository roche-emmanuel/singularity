...
Task::Task(const char *type, bool gpuTask, unsigned int deadline) :
    completionDate(0), gpuTask(gpuTask), deadline(deadline), predecessorsCompletionDate(1), done(false), expectedDuration(-1.0f)
{
	setName(type);
}
...
bool Task::run()
{
    CHECK_RET(!done,true,"Running a task that is already done.");
    return true;
}
