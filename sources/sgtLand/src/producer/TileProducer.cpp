...
public:
     ...
     */
    set< sgtPtr<Task> > rootDependencies;
    ...
    void restore()
    {
        ...
        addTask(root);
        set< sgtPtr<Task> >::iterator i = rootDependencies.begin();
...
TileProducer::TileProducer(const char* type, const char *taskType, TileCache* cache, bool gpuProducer) :
...
void TileProducer::init(TileCache* cache, bool gpuProducer)
{
    assert(cache != NULL);
    this->cache = cache;
...
vec4f TileProducer::getGpuTileCoords(int level, int tx, int ty, TileCache::Tile **tile)
{
    ...
    sgtPtr<TileStorage> storage = getCache()->getStorage();
...
bool TileProducer::prefetchTile(int level, int tx, int ty)
{
    if (cache->getScheduler() != NULL && cache->getScheduler()->supportsPrefetch(isGpuProducer())) {
        sgtPtr<Task> task = cache->prefetchTile(id, level, tx, ty);
        if (task != NULL) {
...
{
    assert(isGpuProducer());
    sgtPtr<GPUTileStorage> gpuStorage = dynamic_cast<GPUTileStorage*>(getCache()->getStorage());
	sgtPtr<osg::Texture2D> tileMapT = gpuStorage->getTileMap();
    if (tileMapT == NULL) {
        return false;
    }
    if (tileMap == NULL) {
        tileMap = new unsigned char[2 * tileMapT->getTextureWidth()];
    }
    ...
    if (k > 2) {
        ...
        for (int i = 0; i < tileMapT->getTextureWidth(); ++i) {
            tileMap[2 * i] = 0;
            tileMap[2 * i + 1] = 0;
        }
...
void TileProducer::getReferencedProducers(vector< sgtPtr<TileProducer> > &producers) const
{
}
...
TileLayer* TileProducer::getLayer(int index) const
{
...
void TileProducer::addLayer(TileLayer* l)
{
    layers.push_back(l);
}

sgtPtr<Task> TileProducer::startCreateTile(int level, int tx, int ty, unsigned int deadline, sgtPtr<Task> task, sgtPtr<TaskGraph> owner)
{
    for (int i = 0; i < (int) layers.size(); i++) {
        layers[i]->startCreateTile(level, tx, ty, deadline, task, owner);
    }
    return owner == NULL ? task : dynamic_cast<Task*>(owner.get());
}
...
sgtPtr<Task> TileProducer::createTile(int level, int tx, int ty, TileStorage::Slot *data, unsigned int deadline, sgtPtr<Task> old)
{
    assert(data != NULL);
    if (old != NULL) {
        sgtPtr<CreateTileTaskGraph> r = dynamic_cast<CreateTileTaskGraph*>(old.get());
        if (r != NULL) {
            r->restore();
        } else {
            assert(dynamic_cast<CreateTile*>(old.get()) != NULL);
        }
        return old;
    }
    sgtPtr<CreateTile> t = new CreateTile(this, level, tx, ty, data, deadline);
    sgtPtr<Task> r = startCreateTile(level, tx, ty, deadline, t, NULL);
...
sgtPtr<TaskGraph> TileProducer::createTaskGraph(sgtPtr<Task> task)
{
    sgtPtr<CreateTile> t = sgtCast(task.get(),CreateTile);
    assert(t != NULL);
    sgtPtr<CreateTileTaskGraph> r = new CreateTileTaskGraph(this);
    r->addTask(t);
    r->root = t.get();
    t->parent = r.get();
    return r;
