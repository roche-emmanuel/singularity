...
TileCache::Tile::Tile(int producerId, int level, int tx, int ty, Task* task, TileStorage::Slot *data) :
    producerId(producerId), level(level), tx(tx), ty(ty), task(task), data(data), users(0)
{
    THROW_IF(data == NULL,"Invalid slot in tile constructor.");
}
...
TileStorage::Slot *TileCache::Tile::getData(bool check)
{
    ...
    CHECK_RET(isDone || !check,NULL,"Cannot get data.");
    CHECK_RET(getTId() == data->id || !check,NULL,"Invalid TId.");
...
TileCache::TileCache(TileStorage* storage,  std::string name, Scheduler* scheduler)
{
...
void TileCache::init(TileStorage* storage, std::string name, Scheduler* scheduler)
{
...
TileCache::~TileCache()
{
    ...
    // empty at this point
    CHECK(usedTiles.size() == 0,"Some tiles are still in use ?!");
    unusedTiles.clear();
...
{
    CHECK_RET(producers.find(producerId) != producers.end(),NULL,"Could not find producer id.");
...
{
	CHECK_RET(producers.find(producerId) != producers.end(),NULL,"Cannot find producer by id: "<< producerId);
    OpenThreads::ScopedLock<OpenThreads::Mutex> lock(mutex);
    ...
    if (i == usedTiles.end()) {
		...
		trDEBUG("TileCache", name << ": tiles: " <<usedTiles.size()<<" used, " << unusedTiles.size()<<" reusable, total " << storage->getCapacity());

    } else {
        // requested tile found in used tiles list -> nothing to do
        t = i->second;
        CHECK_RET(t->producerId == producerId && t->level == level && t->tx == tx && t->ty == ty,NULL,"Could not find tile properly.");
    }
    if (t != NULL) {
        if (users != NULL) {
            *users = t->users;
...
sgtPtr<Task> TileCache::prefetchTile(int producerId, int level, int tx, int ty)
{
    CHECK_RET(producers.find(producerId) != producers.end(),NULL,"Could not find producer by id "<<producerId);
    OpenThreads::ScopedLock<OpenThreads::Mutex> lock(mutex);
    ...
    if (usedTiles.find(id) == usedTiles.end()) {
        if (unusedTiles.find(id) == unusedTiles.end()) {
            ...
            if (data == NULL && unusedTiles.size() > 0) {
                ...
                Tile *t = *li;
                data = t->data;
                CHECK_RET(data != NULL,NULL,"Invalid tile data.");
                unusedTiles.erase(t->getTId());
                unusedTilesOrder.erase(li);
                deletedTiles.insert(make_pair(t->getTId(), t->task.get()));
                delete t;
...
int TileCache::putTile(Tile *t)
{
    ...
    if (t->users == 0) {
        ...
        map<Tile::TId, Tile*>::iterator i = usedTiles.find(id);
        CHECK_RET(i != usedTiles.end() && i->second == t,0,"Invalid tile found");
        usedTiles.erase(i);
        // adds it to the unused tiles list
        CHECK_RET(unusedTiles.find(id) == unusedTiles.end(),0,"Should not find tile here.");
        list<Tile*>::iterator li = unusedTilesOrder.insert(unusedTilesOrder.end(), t);
        unusedTiles[id] = li;
        /*if (Logger::DEBUG_LOGGER != NULL) {
            ostringstream oss;
...
void TileCache::createTileTaskDeleted(int producerId, int level, int tx, int ty)
{
    ...
    OpenThreads::ScopedLock<OpenThreads::Mutex> lock(mutex);
    map<Tile::TId, Task*>::iterator i = deletedTiles.find(id);
    CHECK(i != deletedTiles.end(),"Could not find deleted tile.");
    deletedTiles.erase(i);
