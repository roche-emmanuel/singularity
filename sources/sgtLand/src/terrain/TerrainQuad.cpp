...
{
	LandManager::Visibility v = parent == NULL ? LandManager::PARTIALLY_VISIBLE : parent->visible;
	if (v == LandManager::PARTIALLY_VISIBLE) {
	...
	if (visible != LandManager::INVISIBLE && occluded) {
		occluded = _owner->isOccluded(box3d(ox, ox + l, oy, oy + l, zmin, zmax));
		if (occluded) {
			visible = LandManager::INVISIBLE;
		}
	}

	double ground = _owner->groundHeightAtCamera;
	float dist = _owner->getCameraDist(box3d(ox, ox + l, oy, oy + l, min(0.0, ground), max(0.0, ground)));

	if ((_owner->splitInvisibleQuads || visible != LandManager::INVISIBLE) && dist < l * _owner->getSplitDistance() && level < _owner->maxLevel) {
		if (isLeaf()) {
			subdivide();
		}
		...
		if (visible != LandManager::INVISIBLE) {
