#ifndef _SGTLAND_TILE_PRODUCER_H_
#define _SGTLAND_TILE_PRODUCER_H_
 ...
 */
class SGTLAND_EXPORT TileProducer : public sgt::Object
{
public:
    /**
...
protected:
     ...
     * @param gpuProducer true if this %producer produces textures on GPU.
     */
    void init(TileCache* cache, bool gpuProducer);
     ...
     */
    virtual sgtPtr<Task> startCreateTile(int level, int tx, int ty, unsigned int deadline, sgtPtr<Task> task, sgtPtr<TaskGraph> owner);
     ...
     */
    sgtPtr<TaskGraph> createTaskGraph(sgtPtr<Task> task);
...
private:
     ...
     */
    std::vector< sgtPtr<TileLayer> > layers;
     ...
     */
    sgtPtr<Task> createTile(int level, int tx, int ty, TileStorage::Slot *data, unsigned int deadline, sgtPtr<Task> old);
