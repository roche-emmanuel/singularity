#ifndef _SGTLAND_TILE_CACHE_H_
#define _SGTLAND_TILE_CACHE_H_
 ...
 */
class SGTLAND_EXPORT TileCache : public sgt::Object
{
public:
    /**
    ...
    public:
         ...
         */
        Tile(int producerId, int level, int tx, int ty, Task* task, TileStorage::Slot *data);
     ...
     *      disabled.
     */
    TileCache(TileStorage* storage, std::string name, Scheduler* scheduler = NULL);
     ...
     * @param ty the tile's quadtree y coordinate.
     */
    sgtPtr<Task> prefetchTile(int producerId, int level, int tx, int ty);
...
protected:
     ...
     *      disabled.
     */
    void init(TileStorage* storage, std::string name, Scheduler* scheduler = NULL);
