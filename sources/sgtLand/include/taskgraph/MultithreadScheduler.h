#ifndef _SGT_MULTITHREAD_SCHEDULER_H_
#define _SGT_MULTITHREAD_SCHEDULER_H_
 ...
 * @ingroup taskgraph
 */
class SGTLAND_EXPORT MultithreadScheduler : public Scheduler
{
public:
    /**
    ...
    virtual void reschedule(Task* task, Task::reason r, unsigned int deadline);
...
private:
     ...
     */
    struct taskSort : public std::less< sgtPtr<Task> >
    {
        bool operator()(const sgtPtr<Task> x, const sgtPtr<Task> y) const;
    };
     ...
     */
    typedef std::map<taskKey, std::set<sgtPtr<Task>, taskSort>, taskKeySort> SortedTaskSet;
     ...
     */
    void addFlattenedTask(Task* t, TaskSet& addedTasks);
     ...
     */
    void addFlattenedDependency(Task* src, Task* dst);
     ...
     */
    void setDeadline(Task* t, unsigned int deadline, TaskSet& visited);
     ...
     *      previous execution.
     */
    void taskDone(Task* t, bool changes);
     ...
     */
    static Task* getTask(SortedTaskSet &s, void *previousContext);
     ...
     */
    static void insertTask(SortedTaskSet &s, Task* t);
     ...
     */
    static bool removeTask(SortedTaskSet &s, Task* t);
};
