#ifndef _SGT_TASK_GRAPH_H_
#define _SGT_TASK_GRAPH_H_
...
public:
     ...
     */
    typedef SetIterator< sgtPtr<Task> > TaskIterator;
     ...
     */
    TaskIterator getDependencies(Task* t);
     ...
     * @param t a sub task of this task graph.
     */
    TaskIterator getInverseDependencies(Task* t);
     ...
     */
    void addDependency(Task* src, Task* dst);
     ...
     * @param dst a sub task of this graph that must be executed before src.
     */
    void removeDependency(Task* src, Task* dst);
     ...
     */
    void removeAndGetDependencies(Task* src, TaskSet& deletedDependencies);
     ...
     */
    virtual void taskStateChanged(Task* t, bool done, Task::reason r);

    /**
     * Notifies the listeners of this task that the completion date of a sub task
     * without successors has changed.
     */
    virtual void completionDateChanged(Task* t, unsigned int date);
...
private:
    TaskSet allTasks; ///< all the tasks of this graph

    TaskSet firstTasks; ///< the tasks without predecessors

    TaskSet lastTasks; ///< the tasks without successors

    TaskSet flattenedFirstTasks; ///< the primitive tasks without predecessors

    TaskSet flattenedLastTasks; ///< the primitive tasks without successors
