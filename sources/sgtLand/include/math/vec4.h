...
template <typename type>
inline type vec4<type>::dot(const vec3<type>& v) const
{
    return (x*v.x + y*v.y + z*v.z + w);
}

template <typename type>
inline type vec4<type>::dot(const vec4<type>& v) const
{
    return (x*v.x + y*v.y + z*v.z + w*v.w);
}
...
inline sgt::vec4<vecType> operator*(const scalarType scalar, const sgt::vec4<vecType> &v)
{
