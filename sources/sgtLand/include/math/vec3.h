...
template <>
inline int vec3<int>::length() const
{
...
template <typename type>
inline type vec3<type>::dot(const vec3<type>& v) const
{
    return (x*v.x + y*v.y + z*v.z);
}
...
template <>
inline vec3<int> vec3<int>::normalize() const
{
    double length = sqrt((double)(x * x + y * y + z * z));
    double invLength = 1.0 / length;
    return vec3((int)(x * invLength), (int)(y * invLength), (int)(z * invLength));
}
...
template <>
inline vec3<int> vec3<int>::normalize(int l) const
{
    double length = sqrt((double)(x * x + y * y + z * z));
...
inline sgt::vec3<vecType> operator*(const scalarType scalar, const sgt::vec3<vecType> &v)
{
