...
template <>
inline vec2<int> vec2<int>::normalize() const
{
    double length = sqrt((double)(x * x + y * y));
    double invLength = 1.0 / length;
    return vec2((int)(x * invLength), (int)(y * invLength));
}

template <typename type>
inline vec2<type> vec2<type>::normalize(type l) const
{
    type length = sqrt((type)(x * x + y * y));
    type invLength = l / length;
    return vec2(x * invLength, y * invLength);
}

template <>
inline vec2<int> vec2<int>::normalize(int l) const
{
    double length = sqrt((double)(x * x + y * y));
...
inline sgt::vec2<vecType> operator*(const scalarType scalar, const sgt::vec2<vecType> &v)
{
