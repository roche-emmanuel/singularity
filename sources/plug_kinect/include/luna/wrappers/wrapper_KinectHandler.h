#ifndef _WRAPPERS_WRAPPER_KINECTHANDLER_H_
#define _WRAPPERS_WRAPPER_KINECTHANDLER_H_
...
class wrapper_KinectHandler : public KinectHandler, public luna_wrapper_base {

public:
		

	~wrapper_KinectHandler() {
		logDEBUG3("Calling delete function for wrapper KinectHandler");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((KinectHandler*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_KinectHandler(lua_State* L, lua_Table* dum) 
		: KinectHandler(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((KinectHandler*)this);
			_obj.callFunction<void>();
		}
	};
	...
	bool initialize(int kid) {
		...
		return KinectHandler::initialize(kid);
	};
	...
	bool handleJoints(NUI_SKELETON_DATA & skeletonData) {
		...
		return KinectHandler::handleJoints(skeletonData);
	};
	...
	bool handleImages(NUI_IMAGE_FRAME & imageFrame) {
		...
		return KinectHandler::handleImages(imageFrame);
	};
	...
	bool handle(const osgGA::GUIEventAdapter & ea, osgGA::GUIActionAdapter & aa) {
		...
		return KinectHandler::handle(ea, aa);
	};
	...
	void handleCurrentData() {
		...
		return KinectHandler::handleCurrentData();
	};
