...
public:
	...
	~wrapper_WebViewListener_View() {
		logDEBUG3("Calling delete function for wrapper WebViewListener_View");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((WebViewListener::View*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_WebViewListener_View(lua_State* L, lua_Table* dum) 
		: WebViewListener::View(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((WebViewListener::View*)this);
			_obj.callFunction<void>();
		}
	};
