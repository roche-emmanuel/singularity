...
public:
	...
	// bool ork::half::isNaN() const
	static int _bind_isNaN(lua_State *L) {
		if (!_lg_typecheck_isNaN(L)) {
			luaL_error(L, "luna typecheck failed in bool ork::half::isNaN() const function, expected prototype:\nbool ork::half::isNaN() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		ork::half* self=(Luna< ork::half >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool ork::half::isNaN() const. Got : '%s'\n%s",typeid(Luna< ork::half >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
...
luna_RegType LunaTraits< ork::half >::methods[] = {
	...
	{"isNaN", &luna_wrapper_ork_half::_bind_isNaN},
