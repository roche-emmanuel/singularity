...
public:
	...
	inline static bool _lg_typecheck_setSampler(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,2,74539779) ) return false;
		return true;
	}
	...
	// ptr< ork::Sampler > ork::UniformSampler::getSampler() const
	static int _bind_getSampler(lua_State *L) {
		if (!_lg_typecheck_getSampler(L)) {
			luaL_error(L, "luna typecheck failed in ptr< ork::Sampler > ork::UniformSampler::getSampler() const function, expected prototype:\nptr< ork::Sampler > ork::UniformSampler::getSampler() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		ork::UniformSampler* self=Luna< ork::Object >::checkSubType< ork::UniformSampler >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call ptr< ork::Sampler > ork::UniformSampler::getSampler() const. Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		ptr< ork::Sampler > lret = self->getSampler();
		Luna< ork::Sampler >::push(L,lret.get(),false);

		return 1;
	}

	// void ork::UniformSampler::setSampler(const ptr< ork::Sampler > sampler)
	static int _bind_setSampler(lua_State *L) {
		if (!_lg_typecheck_setSampler(L)) {
			luaL_error(L, "luna typecheck failed in void ork::UniformSampler::setSampler(const ptr< ork::Sampler > sampler) function, expected prototype:\nvoid ork::UniformSampler::setSampler(const ptr< ork::Sampler > sampler)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}

		ptr< ork::Sampler > sampler = Luna< ork::Object >::checkSubType< ork::Sampler >(L,2);

		ork::UniformSampler* self=Luna< ork::Object >::checkSubType< ork::UniformSampler >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::UniformSampler::setSampler(const ptr< ork::Sampler >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// ptr< ork::Texture > ork::UniformSampler::get() const
	static int _bind_get(lua_State *L) {
		if (!_lg_typecheck_get(L)) {
			luaL_error(L, "luna typecheck failed in ptr< ork::Texture > ork::UniformSampler::get() const function, expected prototype:\nptr< ork::Texture > ork::UniformSampler::get() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		ork::UniformSampler* self=Luna< ork::Object >::checkSubType< ork::UniformSampler >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call ptr< ork::Texture > ork::UniformSampler::get() const. Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		ptr< ork::Texture > lret = self->get();
		Luna< ork::Texture >::push(L,lret.get(),false);

		return 1;
	}

	// void ork::UniformSampler::set(ptr< ork::Texture > value)
	static int _bind_set(lua_State *L) {
		if (!_lg_typecheck_set(L)) {
			luaL_error(L, "luna typecheck failed in void ork::UniformSampler::set(ptr< ork::Texture > value) function, expected prototype:\nvoid ork::UniformSampler::set(ptr< ork::Texture > value)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}

		ptr< ork::Texture > value = Luna< ork::Object >::checkSubType< ork::Texture >(L,2);

		ork::UniformSampler* self=Luna< ork::Object >::checkSubType< ork::UniformSampler >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::UniformSampler::set(ptr< ork::Texture >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->set(value);
	...
	// void ork::UniformSampler::setValue(ptr< Value > v)
	static int _bind_setValue(lua_State *L) {
		if (!_lg_typecheck_setValue(L)) {
			luaL_error(L, "luna typecheck failed in void ork::UniformSampler::setValue(ptr< Value > v) function, expected prototype:\nvoid ork::UniformSampler::setValue(ptr< Value > v)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}

		ptr< Value > v = Luna< ork::Object >::checkSubType< Value >(L,2);

		ork::UniformSampler* self=Luna< ork::Object >::checkSubType< ork::UniformSampler >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::UniformSampler::setValue(ptr< Value >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->setValue(v);
	...
	// void ork::UniformSampler::base_setValue(ptr< Value > v)
	static int _bind_base_setValue(lua_State *L) {
		if (!_lg_typecheck_base_setValue(L)) {
			luaL_error(L, "luna typecheck failed in void ork::UniformSampler::base_setValue(ptr< Value > v) function, expected prototype:\nvoid ork::UniformSampler::base_setValue(ptr< Value > v)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}
		...
		ork::UniformSampler* self=Luna< ork::Object >::checkSubType< ork::UniformSampler >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::UniformSampler::base_setValue(ptr< Value >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
