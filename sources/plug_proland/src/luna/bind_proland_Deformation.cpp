...
public:
	...
	static int _cast_from_Object(lua_State *L) {
		...
		//proland::Deformation* ptr= dynamic_cast< proland::Deformation* >(Luna< osg::Referenced >::check(L,1));
		proland::Deformation* ptr= luna_caster< osg::Referenced, proland::Deformation >::cast(Luna< osg::Referenced >::check(L,1));
		if(!ptr)
			return 0;
	...
	// void proland::Deformation::setUniforms(proland::TerrainNode * n) const
	static int _bind_setUniforms(lua_State *L) {
		if (!_lg_typecheck_setUniforms(L)) {
			luaL_error(L, "luna typecheck failed in void proland::Deformation::setUniforms(proland::TerrainNode * n) const function, expected prototype:\nvoid proland::Deformation::setUniforms(proland::TerrainNode * n) const\nClass arguments details:\narg 1 ID = 80168245\n\n%s",luna_dumpStack(L).c_str());
		}

		proland::TerrainNode* n=(Luna< osg::Referenced >::checkSubType< proland::TerrainNode >(L,2));

		proland::Deformation* self=Luna< osg::Referenced >::checkSubType< proland::Deformation >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void proland::Deformation::setUniforms(proland::TerrainNode *) const. Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->setUniforms(n);
	...
	// void proland::Deformation::base_setUniforms(proland::TerrainNode * n) const
	static int _bind_base_setUniforms(lua_State *L) {
		if (!_lg_typecheck_base_setUniforms(L)) {
			luaL_error(L, "luna typecheck failed in void proland::Deformation::base_setUniforms(proland::TerrainNode * n) const function, expected prototype:\nvoid proland::Deformation::base_setUniforms(proland::TerrainNode * n) const\nClass arguments details:\narg 1 ID = 80168245\n\n%s",luna_dumpStack(L).c_str());
		}
		...
		proland::Deformation* self=Luna< osg::Referenced >::checkSubType< proland::Deformation >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void proland::Deformation::base_setUniforms(proland::TerrainNode *) const. Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
...
const char* LunaTraits< proland::Deformation >::parents[] = {"sgt.Object", 0};
const int LunaTraits< proland::Deformation >::hash = 42574160;
const int LunaTraits< proland::Deformation >::uniqueIDs[] = {50169651,0};
...
luna_ConverterType LunaTraits< proland::Deformation >::converters[] = {
	{"sgt::Object", &luna_wrapper_proland_Deformation::_cast_from_Object},
	{0,0}
};
