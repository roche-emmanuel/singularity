...
public:
	...
	static int _bind_setValue(lua_State *L) {
		if (!_lg_typecheck_setValue(L)) {
			luaL_error(L, "luna typecheck failed in void ork::UniformSubroutine::setValue(ptr< Value > v) function, expected prototype:\nvoid ork::UniformSubroutine::setValue(ptr< Value > v)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}
		...
		ork::UniformSubroutine* self=Luna< ork::Object >::checkSubType< ork::UniformSubroutine >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::UniformSubroutine::setValue(ptr< Value >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->setValue(v);
	...
	// void ork::UniformSubroutine::base_setValue(ptr< Value > v)
	static int _bind_base_setValue(lua_State *L) {
		if (!_lg_typecheck_base_setValue(L)) {
			luaL_error(L, "luna typecheck failed in void ork::UniformSubroutine::base_setValue(ptr< Value > v) function, expected prototype:\nvoid ork::UniformSubroutine::base_setValue(ptr< Value > v)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}
		...
		ork::UniformSubroutine* self=Luna< ork::Object >::checkSubType< ork::UniformSubroutine >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::UniformSubroutine::base_setValue(ptr< Value >). Got : '%s'\n%s",typeid(Luna< ork::Object >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
