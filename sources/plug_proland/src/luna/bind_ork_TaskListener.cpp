...
public:
	...
	// Function binds:
	// void ork::TaskListener::taskStateChanged(ptr< ork::Task > t, bool done, ork::Task::reason r)
	static int _bind_taskStateChanged(lua_State *L) {
		if (!_lg_typecheck_taskStateChanged(L)) {
			luaL_error(L, "luna typecheck failed in void ork::TaskListener::taskStateChanged(ptr< ork::Task > t, bool done, ork::Task::reason r) function, expected prototype:\nvoid ork::TaskListener::taskStateChanged(ptr< ork::Task > t, bool done, ork::Task::reason r)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}

		ptr< ork::Task > t = Luna< ork::Object >::checkSubType< ork::Task >(L,2);
		bool done=(bool)(lua_toboolean(L,3)==1);
		ork::Task::reason r=(ork::Task::reason)lua_tointeger(L,4);

		ork::TaskListener* self=(Luna< ork::TaskListener >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::TaskListener::taskStateChanged(ptr< ork::Task >, bool, ork::Task::reason). Got : '%s'\n%s",typeid(Luna< ork::TaskListener >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// void ork::TaskListener::completionDateChanged(ptr< ork::Task > t, unsigned int date)
	static int _bind_completionDateChanged(lua_State *L) {
		if (!_lg_typecheck_completionDateChanged(L)) {
			luaL_error(L, "luna typecheck failed in void ork::TaskListener::completionDateChanged(ptr< ork::Task > t, unsigned int date) function, expected prototype:\nvoid ork::TaskListener::completionDateChanged(ptr< ork::Task > t, unsigned int date)\nClass arguments details:\narg 1 ID = [unknown]\n\n%s",luna_dumpStack(L).c_str());
		}
		...
		ork::TaskListener* self=(Luna< ork::TaskListener >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void ork::TaskListener::completionDateChanged(ptr< ork::Task >, unsigned int). Got : '%s'\n%s",typeid(Luna< ork::TaskListener >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
