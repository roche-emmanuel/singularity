...
void generateWavesSpectrum(ptr<Texture2D> spectrum12Tex, ptr<Texture2D> spectrum34Tex)
{
    ...
    spectrum12Tex->setSubImage(0, 0, 0, FFT_SIZE, FFT_SIZE, RGBA, ork::FLOAT, Buffer::Parameters(), CPUBuffer(spectrum12));
    spectrum34Tex->setSubImage(0, 0, 0, FFT_SIZE, FFT_SIZE, RGBA, ork::FLOAT, Buffer::Parameters(), CPUBuffer(spectrum34));
}
...
void computeSlopeVariances(ptr<FrameBuffer> fbo, ptr<Program> variances, ptr<Texture3D> variancesTex)
{
    ...
    variancesTex->getImage(0, RED, ork::FLOAT, data);
    ...
    ptr<Module> brdfShader)
{
    ...
    spectrum12 = new Texture2D(FFT_SIZE, FFT_SIZE, RGBA16F, RGBA, ork::FLOAT,
        Texture::Parameters().min(NEAREST).mag(NEAREST).wrapS(REPEAT).wrapT(REPEAT),
        Buffer::Parameters(), CPUBuffer(NULL));
    spectrum34 = new Texture2D(FFT_SIZE, FFT_SIZE, RGBA16F, RGBA, ork::FLOAT,
        Texture::Parameters().min(NEAREST).mag(NEAREST).wrapS(REPEAT).wrapT(REPEAT),
        Buffer::Parameters(), CPUBuffer(NULL));
    slopeVariances = new Texture3D(N_SLOPE_VARIANCE, N_SLOPE_VARIANCE, N_SLOPE_VARIANCE, R16F, RED, ork::FLOAT,
        Texture::Parameters().min(NEAREST).mag(NEAREST).wrapS(CLAMP_TO_EDGE).wrapT(CLAMP_TO_EDGE).wrapR(CLAMP_TO_EDGE),
        Buffer::Parameters(), CPUBuffer(NULL));
    ffta = new Texture2DArray(FFT_SIZE, FFT_SIZE, 5, RGBA16F, RGBA, ork::FLOAT,
        Texture::Parameters().min(LINEAR_MIPMAP_LINEAR).mag(LINEAR).wrapS(REPEAT).wrapT(REPEAT).maxAnisotropyEXT(16.0f),
        Buffer::Parameters(), CPUBuffer(NULL));
    fftb = new Texture2DArray(FFT_SIZE, FFT_SIZE, 5, RGBA16F, RGBA, ork::FLOAT,
        Texture::Parameters().min(LINEAR_MIPMAP_LINEAR).mag(LINEAR).wrapS(REPEAT).wrapT(REPEAT).maxAnisotropyEXT(16.0f),
        Buffer::Parameters(), CPUBuffer(NULL));
    float *data = computeButterflyLookupTexture();
    ptr<Texture2D> butterfly = new Texture2D(FFT_SIZE, PASSES, RGBA16F, RGBA, ork::FLOAT,
