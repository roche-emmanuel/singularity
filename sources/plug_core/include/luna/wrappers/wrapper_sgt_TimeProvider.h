...
public:
	...
	~wrapper_sgt_TimeProvider() {
		logDEBUG3("Calling delete function for wrapper sgt_TimeProvider");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((sgt::TimeProvider*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_sgt_TimeProvider(lua_State* L, lua_Table* dum) 
		: sgt::TimeProvider(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((sgt::TimeProvider*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_sgt_TimeProvider(lua_State* L, lua_Table* dum, const sgt::TimeProvider & tm, const osg::CopyOp & co = sgtCopyOp::SHALLOW_COPY) 
		: sgt::TimeProvider(tm, co), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((sgt::TimeProvider*)this);
			_obj.callFunction<void>();
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_sgt_TimeProvider* self=Luna< osg::Referenced >::checkSubType< wrapper_sgt_TimeProvider >(L,1);
		if(!self) {
			luna_printStack(L);
