...
public:
	...
	~wrapper_sgt_LogSink() {
		logDEBUG3("Calling delete function for wrapper sgt_LogSink");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((sgt::LogSink*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_sgt_LogSink(lua_State* L, lua_Table* dum, const std::string & name = "") 
		: sgt::LogSink(name), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((sgt::LogSink*)this);
			_obj.callFunction<void>();
		}
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_sgt_LogSink* self=Luna< osg::Referenced >::checkSubType< wrapper_sgt_LogSink >(L,1);
		if(!self) {
			luna_printStack(L);
