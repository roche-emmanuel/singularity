#include <plug_common.h>

const char LunaTraits< osg::DeleteHandler >::className[] = "osg_DeleteHandler";
const char LunaTraits< osg::DeleteHandler >::fullName[] = "osg::DeleteHandler";
const char LunaTraits< osg::DeleteHandler >::moduleName[] = "osg";
const int LunaTraits< osg::DeleteHandler >::hash = 1576447;
const int LunaTraits< osg::DeleteHandler >::uniqueIDs[] = {1576447,0};

const char LunaTraits< osg::Observer >::className[] = "osg_Observer";
const char LunaTraits< osg::Observer >::fullName[] = "osg::Observer";
const char LunaTraits< osg::Observer >::moduleName[] = "osg";
const int LunaTraits< osg::Observer >::hash = 58375525;
const int LunaTraits< osg::Observer >::uniqueIDs[] = {58375525,0};

const char LunaTraits< osg::ObserverSet >::className[] = "osg_ObserverSet";
const char LunaTraits< osg::ObserverSet >::fullName[] = "osg::ObserverSet";
const char LunaTraits< osg::ObserverSet >::moduleName[] = "osg";
const int LunaTraits< osg::ObserverSet >::hash = 65104703;
const int LunaTraits< osg::ObserverSet >::uniqueIDs[] = {65104703,0};

const char LunaTraits< osg::UserDataContainer >::className[] = "osg_UserDataContainer";
const char LunaTraits< osg::UserDataContainer >::fullName[] = "osg::UserDataContainer";
const char LunaTraits< osg::UserDataContainer >::moduleName[] = "osg";
const int LunaTraits< osg::UserDataContainer >::hash = 87260941;
const int LunaTraits< osg::UserDataContainer >::uniqueIDs[] = {87260941,0};

const char LunaTraits< osg::CopyOp >::className[] = "osg_CopyOp";
const char LunaTraits< osg::CopyOp >::fullName[] = "osg::CopyOp";
const char LunaTraits< osg::CopyOp >::moduleName[] = "osg";
const int LunaTraits< osg::CopyOp >::hash = 27134364;
const int LunaTraits< osg::CopyOp >::uniqueIDs[] = {27134364,0};

const char LunaTraits< osg::State >::className[] = "osg_State";
const char LunaTraits< osg::State >::fullName[] = "osg::State";
const char LunaTraits< osg::State >::moduleName[] = "osg";
const int LunaTraits< osg::State >::hash = 89979598;
const int LunaTraits< osg::State >::uniqueIDs[] = {89979598,0};


