...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,12269219) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(posix_time::ptime*)");
		}

		posix_time::ptime* rhs =(Luna< posix_time::ptime >::check(L,2));
...
luna_RegType LunaTraits< posix_time::ptime >::methods[] = {
	...
	{"__eq", &luna_wrapper_posix_time_ptime::_bind___eq},
	{0,0}
};
