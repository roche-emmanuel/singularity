...
public:
	...
	~wrapper_SPK_StraightEmitter() {
		logDEBUG3("Calling delete function for wrapper SPK_StraightEmitter");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::StraightEmitter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_StraightEmitter(lua_State* L, lua_Table* dum, const SPK::Vector3D & direction = SPK::Vector3D (0.0f, 0.0f,-1.0f)) 
		: SPK::StraightEmitter(direction), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::StraightEmitter*)this);
			_obj.callFunction<void>();
		}
	};
