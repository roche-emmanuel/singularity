#ifndef _WRAPPERS_WRAPPER_SPK_GL_GLRENDERER_H_
#define _WRAPPERS_WRAPPER_SPK_GL_GLRENDERER_H_
...
class wrapper_SPK_GL_GLRenderer : public SPK::GL::GLRenderer, public luna_wrapper_base {

public:
		

	~wrapper_SPK_GL_GLRenderer() {
		logDEBUG3("Calling delete function for wrapper SPK_GL_GLRenderer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::GL::GLRenderer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_GL_GLRenderer(lua_State* L, lua_Table* dum) 
		: SPK::GL::GLRenderer(), luna_wrapper_base(L) { 
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return GLRenderer::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return GLRenderer::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return GLRenderer::destroyChildren(keepChildren);
	};
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return GLRenderer::checkBuffers(group);
	};
...
public:
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return GLRenderer::findByName(name);
	};
	...
	void createBuffers(const SPK::Group & group) {
		...
		return GLRenderer::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return GLRenderer::destroyBuffers(group);
	};
	...
	void enableRenderingHint(SPK::RenderingHint renderingHint, bool enable) {
		...
		return GLRenderer::enableRenderingHint(renderingHint, enable);
	};
	...
	void setAlphaTestThreshold(float alphaThreshold) {
		...
		return GLRenderer::setAlphaTestThreshold(alphaThreshold);
	};
	...
	bool isRenderingHintEnabled(SPK::RenderingHint renderingHint) const {
		...
		return GLRenderer::isRenderingHintEnabled(renderingHint);
	};
	...
	void enableBlending(bool blendingEnabled) {
		...
		return GLRenderer::enableBlending(blendingEnabled);
	};
	...
	void setBlending(SPK::BlendingMode blendMode) {
		...
		return GLRenderer::setBlending(blendMode);
	};
	...
	static int _bind_public_prepareBuffers(lua_State *L) {
		...
		wrapper_SPK_GL_GLRenderer* self=Luna< SPK::BufferHandler >::checkSubType< wrapper_SPK_GL_GLRenderer >(L,1);
		if(!self) {
			luna_printStack(L);
