#ifndef _WRAPPERS_WRAPPER_SPK_SPHERICEMITTER_H_
#define _WRAPPERS_WRAPPER_SPK_SPHERICEMITTER_H_
...
class wrapper_SPK_SphericEmitter : public SPK::SphericEmitter, public luna_wrapper_base {

public:
		

	~wrapper_SPK_SphericEmitter() {
		logDEBUG3("Calling delete function for wrapper SPK_SphericEmitter");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::SphericEmitter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_SphericEmitter(lua_State* L, lua_Table* dum, const SPK::Vector3D & direction = SPK::Vector3D (0.0f, 0.0f,-1.0f), float angleA = 0.0f, float angleB = 0.0f) 
		: SPK::SphericEmitter(direction, angleA, angleB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::SphericEmitter*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::SphericEmitter * SPK::SphericEmitter::clone(bool createBase) const
	SPK::SphericEmitter * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::SphericEmitter::clone");
		_obj.pushArg((SPK::SphericEmitter*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::SphericEmitter*>());
	};

	// void SPK::SphericEmitter::generateVelocity(SPK::Particle & particle, float speed) const
	void generateVelocity(SPK::Particle & particle, float speed) const {
		THROW_IF(!_obj.pushFunction("generateVelocity"),"No implementation for abstract function SPK::SphericEmitter::generateVelocity");
		_obj.pushArg((SPK::SphericEmitter*)this);
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return SphericEmitter::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return SphericEmitter::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return SphericEmitter::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return SphericEmitter::propagateUpdateTransform();
	};

	// void SPK::SphericEmitter::innerUpdateTransform()
	void innerUpdateTransform() {
		...
		return SphericEmitter::innerUpdateTransform();
	};
...
public:
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return SphericEmitter::findByName(name);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_SphericEmitter* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_SphericEmitter >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_SphericEmitter* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_SphericEmitter >(L,1);
		if(!self) {
			luna_printStack(L);
