#ifndef _WRAPPERS_WRAPPER_SPK_RANDOMEMITTER_H_
#define _WRAPPERS_WRAPPER_SPK_RANDOMEMITTER_H_
...
class wrapper_SPK_RandomEmitter : public SPK::RandomEmitter, public luna_wrapper_base {

public:
		

	~wrapper_SPK_RandomEmitter() {
		logDEBUG3("Calling delete function for wrapper SPK_RandomEmitter");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::RandomEmitter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_RandomEmitter(lua_State* L, lua_Table* dum) 
		: SPK::RandomEmitter(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::RandomEmitter*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::RandomEmitter * SPK::RandomEmitter::clone(bool createBase) const
	SPK::RandomEmitter * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::RandomEmitter::clone");
		_obj.pushArg((SPK::RandomEmitter*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::RandomEmitter*>());
	};

	// void SPK::RandomEmitter::generateVelocity(SPK::Particle & particle, float speed) const
	void generateVelocity(SPK::Particle & particle, float speed) const {
		THROW_IF(!_obj.pushFunction("generateVelocity"),"No implementation for abstract function SPK::RandomEmitter::generateVelocity");
		_obj.pushArg((SPK::RandomEmitter*)this);
...
protected:
	...
	void innerUpdateTransform() {
		...
		return RandomEmitter::innerUpdateTransform();
	};
	...
	void registerChildren(bool registerAll) {
		...
		return RandomEmitter::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return RandomEmitter::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return RandomEmitter::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return RandomEmitter::propagateUpdateTransform();
	};
...
public:
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return RandomEmitter::findByName(name);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_RandomEmitter* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_RandomEmitter >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_RandomEmitter* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_RandomEmitter >(L,1);
		if(!self) {
			luna_printStack(L);
