#ifndef _WRAPPERS_WRAPPER_SPK_NORMALEMITTER_H_
#define _WRAPPERS_WRAPPER_SPK_NORMALEMITTER_H_
...
class wrapper_SPK_NormalEmitter : public SPK::NormalEmitter, public luna_wrapper_base {
...
public:
	...
	~wrapper_SPK_NormalEmitter() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::NormalEmitter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_NormalEmitter(lua_State* L, lua_Table* dum, SPK::Zone * normalZone = NULL, bool inverted = false) 
		: SPK::NormalEmitter(normalZone, inverted), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::NormalEmitter*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::NormalEmitter * SPK::NormalEmitter::clone(bool createBase) const
	SPK::NormalEmitter * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::NormalEmitter::clone");
		_obj.pushArg((SPK::NormalEmitter*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::NormalEmitter*>());
	};

	// void SPK::NormalEmitter::generateVelocity(SPK::Particle & particle, float speed) const
	void generateVelocity(SPK::Particle & particle, float speed) const {
		THROW_IF(!_obj.pushFunction("generateVelocity"),"No implementation for abstract function SPK::NormalEmitter::generateVelocity");
		_obj.pushArg((SPK::NormalEmitter*)this);
...
protected:
	...
	void innerUpdateTransform() {
		...
		return NormalEmitter::innerUpdateTransform();
	};
	...
	void propagateUpdateTransform() {
		...
		return NormalEmitter::propagateUpdateTransform();
	};

	// void SPK::NormalEmitter::registerChildren(bool registerAll)
	void registerChildren(bool registerAll) {
		...
		return NormalEmitter::registerChildren(registerAll);
	};

	// void SPK::NormalEmitter::copyChildren(const SPK::Registerable & object, bool createBase)
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return NormalEmitter::copyChildren(object, createBase);
	};

	// void SPK::NormalEmitter::destroyChildren(bool keepChildren)
	void destroyChildren(bool keepChildren) {
		...
		return NormalEmitter::destroyChildren(keepChildren);
	};
...
public:
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_NormalEmitter* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_NormalEmitter >(L,1);
		if(!self) {
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_NormalEmitter* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_NormalEmitter >(L,1);
		if(!self) {
