#ifndef _WRAPPERS_WRAPPER_SPK_MODIFIER_H_
#define _WRAPPERS_WRAPPER_SPK_MODIFIER_H_
...
class wrapper_SPK_Modifier : public SPK::Modifier, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Modifier() {
		logDEBUG3("Calling delete function for wrapper SPK_Modifier");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Modifier*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Modifier(lua_State* L, lua_Table* dum, int availableTriggers = SPK::ALWAYS, SPK::ModifierTrigger trigger = SPK::ALWAYS, bool needsIntersection = false, bool needsNormal = false, SPK::Zone * zone = NULL) 
		: SPK::Modifier(availableTriggers, trigger, needsIntersection, needsNormal, zone), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Modifier*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	void modify(SPK::Particle & particle, float deltaTime) const {
		THROW_IF(!_obj.pushFunction("modify"),"No implementation for abstract function SPK::Modifier::modify");
		_obj.pushArg((SPK::Modifier*)this);
...
protected:
	...
	void innerUpdateTransform() {
		...
		return Modifier::innerUpdateTransform();
	};
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return Modifier::checkBuffers(group);
	};
	...
	void registerChildren(bool registerAll) {
		...
		return Modifier::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Modifier::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Modifier::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Modifier::propagateUpdateTransform();
	};
...
public:
	...
	void createBuffers(const SPK::Group & group) {
		...
		return Modifier::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return Modifier::destroyBuffers(group);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_Modifier* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Modifier >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Modifier* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Modifier >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_prepareBuffers(lua_State *L) {
		...
		wrapper_SPK_Modifier* self=Luna< SPK::BufferHandler >::checkSubType< wrapper_SPK_Modifier >(L,1);
		if(!self) {
			luna_printStack(L);
