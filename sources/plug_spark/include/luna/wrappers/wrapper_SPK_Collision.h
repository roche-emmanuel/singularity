#ifndef _WRAPPERS_WRAPPER_SPK_COLLISION_H_
#define _WRAPPERS_WRAPPER_SPK_COLLISION_H_
...
class wrapper_SPK_Collision : public SPK::Collision, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Collision() {
		logDEBUG3("Calling delete function for wrapper SPK_Collision");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Collision*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Collision(lua_State* L, lua_Table* dum, float scale = 1.0f, float elasticity = 1.0f) 
		: SPK::Collision(scale, elasticity), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Collision*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Collision * SPK::Collision::clone(bool createBase) const
	SPK::Collision * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Collision::clone");
		_obj.pushArg((SPK::Collision*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Collision*>());
	};

	// void SPK::Collision::modify(SPK::Particle & particle, float deltaTime) const
	void modify(SPK::Particle & particle, float deltaTime) const {
		THROW_IF(!_obj.pushFunction("modify"),"No implementation for abstract function SPK::Collision::modify");
		_obj.pushArg((SPK::Collision*)this);
		_obj.pushArg(&particle);
...
protected:
	...
	void innerUpdateTransform() {
		...
		return Collision::innerUpdateTransform();
	};
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return Collision::checkBuffers(group);
	};
	...
	void registerChildren(bool registerAll) {
		...
		return Collision::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Collision::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Collision::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Collision::propagateUpdateTransform();
	};
...
public:
	...
	void createBuffers(const SPK::Group & group) {
		...
		return Collision::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return Collision::destroyBuffers(group);
	};
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return Collision::findByName(name);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_Collision* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Collision >(L,1);
		if(!self) {
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Collision* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Collision >(L,1);
		if(!self) {
	...
	static int _bind_public_prepareBuffers(lua_State *L) {
		...
		wrapper_SPK_Collision* self=Luna< SPK::BufferHandler >::checkSubType< wrapper_SPK_Collision >(L,1);
		if(!self) {
