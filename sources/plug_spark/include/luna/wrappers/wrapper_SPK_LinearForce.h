#ifndef _WRAPPERS_WRAPPER_SPK_LINEARFORCE_H_
#define _WRAPPERS_WRAPPER_SPK_LINEARFORCE_H_
...
class wrapper_SPK_LinearForce : public SPK::LinearForce, public luna_wrapper_base {

public:
		

	~wrapper_SPK_LinearForce() {
		logDEBUG3("Calling delete function for wrapper SPK_LinearForce");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::LinearForce*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_LinearForce(lua_State* L, lua_Table* dum, SPK::Zone * zone = NULL, SPK::ModifierTrigger trigger = SPK::INSIDE_ZONE, const SPK::Vector3D & force = SPK::Vector3D (), SPK::ForceFactor type = SPK::FACTOR_NONE, SPK::ModelParam param = SPK::PARAM_SIZE) 
		: SPK::LinearForce(zone, trigger, force, type, param), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::LinearForce*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::LinearForce * SPK::LinearForce::clone(bool createBase) const
	SPK::LinearForce * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::LinearForce::clone");
		_obj.pushArg((SPK::LinearForce*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::LinearForce*>());
	};

	// void SPK::LinearForce::modify(SPK::Particle & particle, float deltaTime) const
	void modify(SPK::Particle & particle, float deltaTime) const {
		THROW_IF(!_obj.pushFunction("modify"),"No implementation for abstract function SPK::LinearForce::modify");
		_obj.pushArg((SPK::LinearForce*)this);
		_obj.pushArg(&particle);
...
protected:
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return LinearForce::checkBuffers(group);
	};
	...
	void registerChildren(bool registerAll) {
		...
		return LinearForce::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return LinearForce::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return LinearForce::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return LinearForce::propagateUpdateTransform();
	};

	// void SPK::LinearForce::innerUpdateTransform()
	void innerUpdateTransform() {
		...
		return LinearForce::innerUpdateTransform();
	};
...
public:
	...
	void createBuffers(const SPK::Group & group) {
		...
		return LinearForce::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return LinearForce::destroyBuffers(group);
	};
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return LinearForce::findByName(name);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_LinearForce* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_LinearForce >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_LinearForce* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_LinearForce >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_prepareBuffers(lua_State *L) {
		...
		wrapper_SPK_LinearForce* self=Luna< SPK::BufferHandler >::checkSubType< wrapper_SPK_LinearForce >(L,1);
		if(!self) {
			luna_printStack(L);
