#ifndef _WRAPPERS_WRAPPER_SPK_SPHERE_H_
#define _WRAPPERS_WRAPPER_SPK_SPHERE_H_
...
class wrapper_SPK_Sphere : public SPK::Sphere, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Sphere() {
		logDEBUG3("Calling delete function for wrapper SPK_Sphere");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Sphere*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Sphere(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (0.0f, 0.0f, 0.0f), float radius = 0.0f) 
		: SPK::Sphere(position, radius), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Sphere*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Sphere * SPK::Sphere::clone(bool createBase) const
	SPK::Sphere * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Sphere::clone");
		_obj.pushArg((SPK::Sphere*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Sphere*>());
	};
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return Sphere::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Sphere::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Sphere::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Sphere::propagateUpdateTransform();
	};
	...
	void innerUpdateTransform() {
		...
		return Sphere::innerUpdateTransform();
	};
...
public:
	...
	void generatePosition(SPK::Particle & particle, bool full) const {
		...
		return Sphere::generatePosition(particle, full);
	};
	...
	void moveAtBorder(SPK::Vector3D & point, bool inside) const {
		...
		return Sphere::moveAtBorder(point, inside);
	};
	...
	static int _bind_public_normalizeOrRandomize(lua_State *L) {
		...
		wrapper_SPK_Sphere* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Sphere >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Sphere* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Sphere >(L,1);
		if(!self) {
			luna_printStack(L);
