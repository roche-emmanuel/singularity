#ifndef _WRAPPERS_WRAPPER_SPK_AABOX_H_
#define _WRAPPERS_WRAPPER_SPK_AABOX_H_
...
class wrapper_SPK_AABox : public SPK::AABox, public luna_wrapper_base {
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return AABox::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return AABox::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return AABox::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return AABox::propagateUpdateTransform();
	};
	...
	void innerUpdateTransform() {
		...
		return AABox::innerUpdateTransform();
	};
...
public:
	...
	void generatePosition(SPK::Particle & particle, bool full) const {
		...
		return AABox::generatePosition(particle, full);
	};
	...
	void moveAtBorder(SPK::Vector3D & point, bool inside) const {
		...
		return AABox::moveAtBorder(point, inside);
	};
	...
	static int _bind_public_normalizeOrRandomize(lua_State *L) {
		...
		wrapper_SPK_AABox* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_AABox >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_AABox* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_AABox >(L,1);
		if(!self) {
			luna_printStack(L);
