#ifndef _WRAPPERS_WRAPPER_SPK_MODEL_H_
#define _WRAPPERS_WRAPPER_SPK_MODEL_H_
...
class wrapper_SPK_Model : public SPK::Model, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Model() {
		logDEBUG3("Calling delete function for wrapper SPK_Model");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Model*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Model(lua_State* L, lua_Table* dum, int enableFlag = SPK::FLAG_RED | SPK::FLAG_GREEN | SPK::FLAG_BLUE, int mutableFlag = SPK::FLAG_NONE, int randomFlag = SPK::FLAG_NONE, int interpolatedFlag = SPK::FLAG_NONE) 
		: SPK::Model(enableFlag, mutableFlag, randomFlag, interpolatedFlag), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Model*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_SPK_Model(lua_State* L, lua_Table* dum, const SPK::Model & model) 
		: SPK::Model(model), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Model*)this);
			_obj.callFunction<void>();
...
private:
	...
	// SPK::Model * SPK::Model::clone(bool createBase) const
	SPK::Model * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Model::clone");
		_obj.pushArg((SPK::Model*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Model*>());
	};
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return Model::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Model::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Model::destroyChildren(keepChildren);
	};
...
public:
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_Model* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Model >(L,1);
		if(!self) {
			luna_printStack(L);
