...
public:
	...
	wrapper_SPK_Vortex(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (), const SPK::Vector3D & direction = SPK::Vector3D (0.0f, 1.0f, 0.0f), float rotationSpeed = 1.0f, float attractionSpeed = 0.0f) : SPK::Vortex(position, direction, rotationSpeed, attractionSpeed), luna_wrapper_base(L) { register_protected_methods(L); };
