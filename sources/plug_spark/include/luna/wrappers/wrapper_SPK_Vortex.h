#ifndef _WRAPPERS_WRAPPER_SPK_VORTEX_H_
#define _WRAPPERS_WRAPPER_SPK_VORTEX_H_
...
class wrapper_SPK_Vortex : public SPK::Vortex, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Vortex() {
		logDEBUG3("Calling delete function for wrapper SPK_Vortex");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Vortex*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Vortex(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (), const SPK::Vector3D & direction = SPK::Vector3D (0.0f, 1.0f, 0.0f), float rotationSpeed = 1.0f, float attractionSpeed = 0.0f) 
		: SPK::Vortex(position, direction, rotationSpeed, attractionSpeed), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Vortex*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Vortex * SPK::Vortex::clone(bool createBase) const
	SPK::Vortex * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Vortex::clone");
		_obj.pushArg((SPK::Vortex*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Vortex*>());
	};

	// void SPK::Vortex::modify(SPK::Particle & particle, float deltaTime) const
	void modify(SPK::Particle & particle, float deltaTime) const {
		THROW_IF(!_obj.pushFunction("modify"),"No implementation for abstract function SPK::Vortex::modify");
		_obj.pushArg((SPK::Vortex*)this);
		_obj.pushArg(&particle);
...
protected:
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return Vortex::checkBuffers(group);
	};
	...
	void registerChildren(bool registerAll) {
		...
		return Vortex::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Vortex::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Vortex::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Vortex::propagateUpdateTransform();
	};

	// void SPK::Vortex::innerUpdateTransform()
	void innerUpdateTransform() {
		...
		return Vortex::innerUpdateTransform();
	};
...
public:
	...
	void createBuffers(const SPK::Group & group) {
		...
		return Vortex::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return Vortex::destroyBuffers(group);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_Vortex* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Vortex >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Vortex* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Vortex >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_prepareBuffers(lua_State *L) {
		...
		wrapper_SPK_Vortex* self=Luna< SPK::BufferHandler >::checkSubType< wrapper_SPK_Vortex >(L,1);
		if(!self) {
			luna_printStack(L);
