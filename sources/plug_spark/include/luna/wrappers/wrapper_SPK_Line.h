#ifndef _WRAPPERS_WRAPPER_SPK_LINE_H_
#define _WRAPPERS_WRAPPER_SPK_LINE_H_
...
class wrapper_SPK_Line : public SPK::Line, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Line() {
		logDEBUG3("Calling delete function for wrapper SPK_Line");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Line*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Line(lua_State* L, lua_Table* dum, const SPK::Vector3D & p0 = SPK::Vector3D (0.0f, 0.0f, 0.0f), const SPK::Vector3D & p1 = SPK::Vector3D (0.0f, 0.0f, 0.0f)) 
		: SPK::Line(p0, p1), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Line*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Line * SPK::Line::clone(bool createBase) const
	SPK::Line * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Line::clone");
		_obj.pushArg((SPK::Line*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Line*>());
	};
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return Line::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Line::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Line::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Line::propagateUpdateTransform();
	};

	// void SPK::Line::innerUpdateTransform()
	void innerUpdateTransform() {
		...
		return Line::innerUpdateTransform();
	};
...
public:
	...
	void generatePosition(SPK::Particle & particle, bool full) const {
		...
		return Line::generatePosition(particle, full);
	};
	...
	void moveAtBorder(SPK::Vector3D & point, bool inside) const {
		...
		return Line::moveAtBorder(point, inside);
	};
	...
	static int _bind_public_normalizeOrRandomize(lua_State *L) {
		...
		wrapper_SPK_Line* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Line >(L,1);
		if(!self) {
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Line* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Line >(L,1);
		if(!self) {
