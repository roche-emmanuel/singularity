...
public:
	...
	wrapper_SPK_Ring(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (0.0f, 0.0f, 0.0f), const SPK::Vector3D & normal = SPK::Vector3D (0.0f, 1.0f, 0.0f), float minRadius = 0.0f, float maxRadius = 1.0f) : SPK::Ring(position, normal, minRadius, maxRadius), luna_wrapper_base(L) { register_protected_methods(L); };
