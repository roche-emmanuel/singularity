...
public:
	...
	wrapper_SPK_Cylinder(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (0.0f, 0.0f, 0.0f), const SPK::Vector3D & direction = SPK::Vector3D (0.0f, 1.0f, 0.0f), float radius = 1.0f, float length = 1.0f) : SPK::Cylinder(position, direction, radius, length), luna_wrapper_base(L) { register_protected_methods(L); };
