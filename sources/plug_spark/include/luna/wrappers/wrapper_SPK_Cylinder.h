#ifndef _WRAPPERS_WRAPPER_SPK_CYLINDER_H_
#define _WRAPPERS_WRAPPER_SPK_CYLINDER_H_
...
class wrapper_SPK_Cylinder : public SPK::Cylinder, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Cylinder() {
		logDEBUG3("Calling delete function for wrapper SPK_Cylinder");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Cylinder*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Cylinder(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (0.0f, 0.0f, 0.0f), const SPK::Vector3D & direction = SPK::Vector3D (0.0f, 1.0f, 0.0f), float radius = 1.0f, float length = 1.0f) 
		: SPK::Cylinder(position, direction, radius, length), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Cylinder*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Cylinder * SPK::Cylinder::clone(bool createBase) const
	SPK::Cylinder * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Cylinder::clone");
		_obj.pushArg((SPK::Cylinder*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Cylinder*>());
	};
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return Cylinder::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Cylinder::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Cylinder::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Cylinder::propagateUpdateTransform();
	};

	// void SPK::Cylinder::innerUpdateTransform()
	void innerUpdateTransform() {
		...
		return Cylinder::innerUpdateTransform();
	};
...
public:
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return Cylinder::findByName(name);
	};
	...
	void generatePosition(SPK::Particle & particle, bool full) const {
		...
		return Cylinder::generatePosition(particle, full);
	};
	...
	void moveAtBorder(SPK::Vector3D & point, bool inside) const {
		...
		return Cylinder::moveAtBorder(point, inside);
	};
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Cylinder* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Cylinder >(L,1);
		if(!self) {
			luna_printStack(L);
