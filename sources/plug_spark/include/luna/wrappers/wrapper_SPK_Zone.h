#ifndef _WRAPPERS_WRAPPER_SPK_ZONE_H_
#define _WRAPPERS_WRAPPER_SPK_ZONE_H_
...
class wrapper_SPK_Zone : public SPK::Zone, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Zone() {
		logDEBUG3("Calling delete function for wrapper SPK_Zone");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Zone*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Zone(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D ()) 
		: SPK::Zone(position), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Zone*)this);
			_obj.callFunction<void>();
		}
	};
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return Zone::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Zone::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Zone::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Zone::propagateUpdateTransform();
	};
	...
	void innerUpdateTransform() {
		...
		return Zone::innerUpdateTransform();
	};
...
public:
	...
	void generatePosition(SPK::Particle & particle, bool full) const {
		THROW_IF(!_obj.pushFunction("generatePosition"),"No implementation for abstract function SPK::Zone::generatePosition");
		_obj.pushArg((SPK::Zone*)this);
	...
	bool contains(const SPK::Vector3D & point) const {
		THROW_IF(!_obj.pushFunction("contains"),"No implementation for abstract function SPK::Zone::contains");
		_obj.pushArg((SPK::Zone*)this);
		_obj.pushArg(&point);
	...
	bool intersects(const SPK::Vector3D & v0, const SPK::Vector3D & v1, SPK::Vector3D * intersection, SPK::Vector3D * normal) const {
		THROW_IF(!_obj.pushFunction("intersects"),"No implementation for abstract function SPK::Zone::intersects");
		_obj.pushArg((SPK::Zone*)this);
		_obj.pushArg(&v0);
		_obj.pushArg(&v1);
	...
	void moveAtBorder(SPK::Vector3D & point, bool inside) const {
		THROW_IF(!_obj.pushFunction("moveAtBorder"),"No implementation for abstract function SPK::Zone::moveAtBorder");
		_obj.pushArg((SPK::Zone*)this);
		_obj.pushArg(&point);
		_obj.pushArg(inside);
		return (_obj.callFunction<void>());
	...
	SPK::Vector3D computeNormal(const SPK::Vector3D & point) const {
		THROW_IF(!_obj.pushFunction("computeNormal"),"No implementation for abstract function SPK::Zone::computeNormal");
		_obj.pushArg((SPK::Zone*)this);
		_obj.pushArg(&point);
	...
	static int _bind_public_normalizeOrRandomize(lua_State *L) {
		...
		wrapper_SPK_Zone* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Zone >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Zone* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Zone >(L,1);
		if(!self) {
			luna_printStack(L);
