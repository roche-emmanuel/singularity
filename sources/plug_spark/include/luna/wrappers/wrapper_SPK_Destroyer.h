#ifndef _WRAPPERS_WRAPPER_SPK_DESTROYER_H_
#define _WRAPPERS_WRAPPER_SPK_DESTROYER_H_
...
class wrapper_SPK_Destroyer : public SPK::Destroyer, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Destroyer() {
		logDEBUG3("Calling delete function for wrapper SPK_Destroyer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Destroyer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Destroyer(lua_State* L, lua_Table* dum, SPK::Zone * zone = NULL, SPK::ModifierTrigger trigger = SPK::INSIDE_ZONE) 
		: SPK::Destroyer(zone, trigger), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Destroyer*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Destroyer * SPK::Destroyer::clone(bool createBase) const
	SPK::Destroyer * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Destroyer::clone");
		_obj.pushArg((SPK::Destroyer*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Destroyer*>());
	};

	// void SPK::Destroyer::modify(SPK::Particle & particle, float deltaTime) const
	void modify(SPK::Particle & particle, float deltaTime) const {
		THROW_IF(!_obj.pushFunction("modify"),"No implementation for abstract function SPK::Destroyer::modify");
		_obj.pushArg((SPK::Destroyer*)this);
	...
	// void SPK::Destroyer::modifyWrongSide(SPK::Particle & particle, bool inside) const
	void modifyWrongSide(SPK::Particle & particle, bool inside) const {
		THROW_IF(!_obj.pushFunction("modifyWrongSide"),"No implementation for abstract function SPK::Destroyer::modifyWrongSide");
		_obj.pushArg((SPK::Destroyer*)this);
		_obj.pushArg(&particle);
		_obj.pushArg(inside);
		return (_obj.callFunction<void>());
...
protected:
	...
	void innerUpdateTransform() {
		...
		return Destroyer::innerUpdateTransform();
	};
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return Destroyer::checkBuffers(group);
	};
	...
	void registerChildren(bool registerAll) {
		...
		return Destroyer::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Destroyer::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Destroyer::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Destroyer::propagateUpdateTransform();
	};
...
public:
	...
	void createBuffers(const SPK::Group & group) {
		...
		return Destroyer::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return Destroyer::destroyBuffers(group);
	};
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return Destroyer::findByName(name);
	};
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_Destroyer* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Destroyer >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Destroyer* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Destroyer >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_prepareBuffers(lua_State *L) {
		...
		wrapper_SPK_Destroyer* self=Luna< SPK::BufferHandler >::checkSubType< wrapper_SPK_Destroyer >(L,1);
		if(!self) {
			luna_printStack(L);
