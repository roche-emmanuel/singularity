#ifndef _WRAPPERS_WRAPPER_SPK_SYSTEM_H_
#define _WRAPPERS_WRAPPER_SPK_SYSTEM_H_
...
class wrapper_SPK_System : public SPK::System, public luna_wrapper_base {

public:
		

	~wrapper_SPK_System() {
		logDEBUG3("Calling delete function for wrapper SPK_System");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::System*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_System(lua_State* L, lua_Table* dum) 
		: SPK::System(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::System*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::System * SPK::System::clone(bool createBase) const
	SPK::System * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::System::clone");
		_obj.pushArg((SPK::System*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::System*>());
	};
...
protected:
	...
	void innerUpdateTransform() {
		...
		return System::innerUpdateTransform();
	};

	// void SPK::System::registerChildren(bool registerAll)
	void registerChildren(bool registerAll) {
		...
		return System::registerChildren(registerAll);
	};

	// void SPK::System::copyChildren(const SPK::Registerable & object, bool createBase)
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return System::copyChildren(object, createBase);
	};

	// void SPK::System::destroyChildren(bool keepChildren)
	void destroyChildren(bool keepChildren) {
		...
		return System::destroyChildren(keepChildren);
	};

	// void SPK::System::propagateUpdateTransform()
	void propagateUpdateTransform() {
		...
		return System::propagateUpdateTransform();
	};
...
public:
	...
	static int _bind_public_registerChild(lua_State *L) {
		...
		wrapper_SPK_System* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_System >(L,1);
		if(!self) {
			luna_printStack(L);
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_System* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_System >(L,1);
		if(!self) {
			luna_printStack(L);
