#ifndef _WRAPPERS_WRAPPER_SPK_OBSTACLE_H_
#define _WRAPPERS_WRAPPER_SPK_OBSTACLE_H_
...
class wrapper_SPK_Obstacle : public SPK::Obstacle, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Obstacle() {
		logDEBUG3("Calling delete function for wrapper SPK_Obstacle");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Obstacle*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Obstacle(lua_State* L, lua_Table* dum, SPK::Zone * zone = NULL, SPK::ModifierTrigger trigger = SPK::INTERSECT_ZONE, float bouncingRatio = 1.0f, float friction = 1.0f) 
		: SPK::Obstacle(zone, trigger, bouncingRatio, friction), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Obstacle*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Obstacle * SPK::Obstacle::clone(bool createBase) const
	SPK::Obstacle * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Obstacle::clone");
		_obj.pushArg((SPK::Obstacle*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Obstacle*>());
	};

	// void SPK::Obstacle::modify(SPK::Particle & particle, float deltaTime) const
	void modify(SPK::Particle & particle, float deltaTime) const {
		THROW_IF(!_obj.pushFunction("modify"),"No implementation for abstract function SPK::Obstacle::modify");
		_obj.pushArg((SPK::Obstacle*)this);
	...
	// void SPK::Obstacle::modifyWrongSide(SPK::Particle & particle, bool inside) const
	void modifyWrongSide(SPK::Particle & particle, bool inside) const {
		THROW_IF(!_obj.pushFunction("modifyWrongSide"),"No implementation for abstract function SPK::Obstacle::modifyWrongSide");
		_obj.pushArg((SPK::Obstacle*)this);
		_obj.pushArg(&particle);
		_obj.pushArg(inside);
		return (_obj.callFunction<void>());
...
protected:
	...
	void innerUpdateTransform() {
		...
		return Obstacle::innerUpdateTransform();
	};
	...
	bool checkBuffers(const SPK::Group & group) {
		...
		return Obstacle::checkBuffers(group);
	};
	...
	void registerChildren(bool registerAll) {
		...
		return Obstacle::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Obstacle::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Obstacle::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Obstacle::propagateUpdateTransform();
	};
...
public:
	...
	void createBuffers(const SPK::Group & group) {
		...
		return Obstacle::createBuffers(group);
	};
	...
	void destroyBuffers(const SPK::Group & group) {
		...
		return Obstacle::destroyBuffers(group);
	};
	...
	SPK::Registerable * findByName(const std::string & name) {
		...
		return Obstacle::findByName(name);
	};
