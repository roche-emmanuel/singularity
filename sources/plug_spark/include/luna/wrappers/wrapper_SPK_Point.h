...
public:
	...
	wrapper_SPK_Point(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (0.0f, 0.0f, 0.0f)) : SPK::Point(position), luna_wrapper_base(L) { register_protected_methods(L); };
...
public:
	...
	static int _bind_public_normalizeOrRandomize(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call static void SPK::Zone::public_normalizeOrRandomize(SPK::Vector3D &)");
		}
