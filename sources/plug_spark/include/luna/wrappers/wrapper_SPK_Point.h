#ifndef _WRAPPERS_WRAPPER_SPK_POINT_H_
#define _WRAPPERS_WRAPPER_SPK_POINT_H_
...
class wrapper_SPK_Point : public SPK::Point, public luna_wrapper_base {

public:
		

	~wrapper_SPK_Point() {
		logDEBUG3("Calling delete function for wrapper SPK_Point");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((SPK::Point*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_SPK_Point(lua_State* L, lua_Table* dum, const SPK::Vector3D & position = SPK::Vector3D (0.0f, 0.0f, 0.0f)) 
		: SPK::Point(position), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((SPK::Point*)this);
			_obj.callFunction<void>();
		}
	};
...
private:
	...
	// SPK::Point * SPK::Point::clone(bool createBase) const
	SPK::Point * clone(bool createBase) const {
		THROW_IF(!_obj.pushFunction("clone"),"No implementation for abstract function SPK::Point::clone");
		_obj.pushArg((SPK::Point*)this);
		_obj.pushArg(createBase);
		return (_obj.callFunction<SPK::Point*>());
	};
...
protected:
	...
	void registerChildren(bool registerAll) {
		...
		return Point::registerChildren(registerAll);
	};
	...
	void copyChildren(const SPK::Registerable & object, bool createBase) {
		...
		return Point::copyChildren(object, createBase);
	};
	...
	void destroyChildren(bool keepChildren) {
		...
		return Point::destroyChildren(keepChildren);
	};
	...
	void propagateUpdateTransform() {
		...
		return Point::propagateUpdateTransform();
	};
	...
	void innerUpdateTransform() {
		...
		return Point::innerUpdateTransform();
	};
...
public:
	...
	void generatePosition(SPK::Particle & particle, bool full) const {
		...
		return Point::generatePosition(particle, full);
	};
	...
	void moveAtBorder(SPK::Vector3D & point, bool inside) const {
		...
		return Point::moveAtBorder(point, inside);
	};
	...
	static int _bind_public_normalizeOrRandomize(lua_State *L) {
		...
		wrapper_SPK_Point* self=Luna< SPK::Registerable >::checkSubType< wrapper_SPK_Point >(L,1);
		if(!self) {
	...
	static int _bind_public_transformPos(lua_State *L) {
		...
		wrapper_SPK_Point* self=Luna< SPK::Transformable >::checkSubType< wrapper_SPK_Point >(L,1);
		if(!self) {
