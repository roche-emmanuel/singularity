...
public:
	...
	static int _bind_getClassName(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call std::string SPK::StaticEmitter::getClassName() const. Got : '%s'\n%s",typeid(Luna< SPK::Registerable >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_base_findByName(lua_State *L) {
		...
		SPK::StaticEmitter* self=Luna< SPK::Registerable >::checkSubType< SPK::StaticEmitter >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call SPK::Registerable * SPK::StaticEmitter::base_findByName(const std::string &). Got : '%s'\n%s",typeid(Luna< SPK::Registerable >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_base_getClassName(lua_State *L) {
		...
		SPK::StaticEmitter* self=Luna< SPK::Registerable >::checkSubType< SPK::StaticEmitter >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call std::string SPK::StaticEmitter::base_getClassName() const. Got : '%s'\n%s",typeid(Luna< SPK::Registerable >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
