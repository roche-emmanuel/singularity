...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(SPK::BufferHandler*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_createBuffers(lua_State *L) {
		...
		SPK::BufferHandler* self=(Luna< SPK::BufferHandler >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void SPK::BufferHandler::createBuffers(const SPK::Group &). Got : '%s'\n%s",typeid(Luna< SPK::BufferHandler >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_destroyBuffers(lua_State *L) {
		...
		SPK::BufferHandler* self=(Luna< SPK::BufferHandler >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void SPK::BufferHandler::destroyBuffers(const SPK::Group &). Got : '%s'\n%s",typeid(Luna< SPK::BufferHandler >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_base_createBuffers(lua_State *L) {
		...
		SPK::BufferHandler* self=(Luna< SPK::BufferHandler >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void SPK::BufferHandler::base_createBuffers(const SPK::Group &). Got : '%s'\n%s",typeid(Luna< SPK::BufferHandler >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_base_destroyBuffers(lua_State *L) {
		...
		SPK::BufferHandler* self=(Luna< SPK::BufferHandler >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void SPK::BufferHandler::base_destroyBuffers(const SPK::Group &). Got : '%s'\n%s",typeid(Luna< SPK::BufferHandler >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
