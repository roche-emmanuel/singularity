...
void ScreenParticleLayer::addNewParticles()
{
    ...
    if (sameView) {
        ...
        if (!depthBufferRead) {
            ...
            ptr<FrameBuffer> fb = SceneManager::getCurrentFrameBuffer();
            fb->readPixels(0, 0, width, height, DEPTH_COMPONENT, PIXEL_FLOAT, Buffer::Parameters(), CPUBuffer(depthArray));
...
void ScreenParticleLayer::getParticleDepths(const vector<ScreenParticle*> &particles)
{
    ...
    if (!useOffscreenDepthBuffer) {
        if (depthBuffer == NULL || depthBuffer->getWidth() != width || depthBuffer->getHeight() != height) {
            depthBuffer = new Texture2D(width, height, DEPTH_COMPONENT32F, DEPTH_COMPONENT,
                PIXEL_FLOAT, Texture::Parameters().wrapS(CLAMP_TO_EDGE).wrapT(CLAMP_TO_EDGE).min(NEAREST).mag(NEAREST), Buffer::Parameters(), CPUBuffer(NULL));
        }
    ...
    if (frameBuffer == NULL) {
        ...
        ptr<Texture2D> result = new Texture2D(maxParticles, 1, R32F,
            RED, PIXEL_FLOAT, Texture::Parameters().wrapS(CLAMP_TO_BORDER).wrapT(CLAMP_TO_BORDER).min(NEAREST).mag(NEAREST), Buffer::Parameters(), CPUBuffer(NULL));
        ...
        mesh = new Mesh<vec3f, unsigned int>(MESH_POINTS, CPU, maxParticles);
    ...
    frameBuffer->readPixels(0, 0, count, 1, RED, PIXEL_FLOAT, Buffer::Parameters(), CPUBuffer(depthArray));
}
