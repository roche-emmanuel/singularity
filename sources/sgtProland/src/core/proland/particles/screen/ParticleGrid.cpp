...
ptr<Texture2D> ParticleGrid::copyToTexture(ptr<ScreenParticleLayer> l, ptr<Texture2D> t, int &pixelsPerCell)
{
    ...
    if (t == NULL || t->getWidth() != width || t->getHeight() != height) {
        t = new Texture2D(width, height, RGBA32F,
            RGBA, PIXEL_FLOAT, Texture::Parameters().wrapS(CLAMP_TO_BORDER).wrapT(CLAMP_TO_BORDER).min(NEAREST).mag(NEAREST), Buffer::Parameters(), CPUBuffer(NULL));
    }
    ...
    t->setSubImage(0, 0, 0, width, height, RGBA, PIXEL_FLOAT, Buffer::Parameters(), CPUBuffer(cellIndexes));
    return t;
}
