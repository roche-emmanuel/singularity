    ...
    Object("TileSamplerZ::State"), storage(storage), cameraSlot(NULL), lastFrame(0)
{
    ...
    fbo->setTextureBuffer(COLOR0, new Texture2D(w, h, RG32F, RG, PIXEL_FLOAT,
            Texture::Parameters().min(NEAREST).mag(NEAREST), Buffer::Parameters(), CPUBuffer()), 0);
    fbo->setTextureBuffer(COLOR1, new Texture2D(w, h, RG32F, RG, PIXEL_FLOAT,
            Texture::Parameters().min(NEAREST).mag(NEAREST), Buffer::Parameters(), CPUBuffer()), 0);
    int pass = 0;
    while (h != 1) {
        h = h / 4 + (h % 4 == 0 ? 0 : 1);
...
ptr<Task> TileSamplerZ::update(ptr<SceneManager> scene, ptr<TerrainQuad> root)
{
    ...
    state->tileReadback->readback(state->fbo, 0, 0, MAX_MIPMAP_PER_FRAME, 1, RG, PIXEL_FLOAT, new TileCallback(targets, camera != vec2i::ZERO));
