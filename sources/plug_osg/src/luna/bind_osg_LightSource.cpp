...
public:
	...
	// void osg::LightSource::setStateSetModes(osg::StateSet & , unsigned int ) const
	static int _bind_setStateSetModes(lua_State *L) {
		if (!_lg_typecheck_setStateSetModes(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in void osg::LightSource::setStateSetModes(osg::StateSet & , unsigned int ) const function, expected prototype:\nvoid osg::LightSource::setStateSetModes(osg::StateSet & , unsigned int ) const\nClass arguments details:\narg 1 ID = 50169651\n");
		}
	...
	static int _bind_computeBound(lua_State *L) {
		if (!_lg_typecheck_computeBound(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in osg::BoundingSpheref osg::LightSource::computeBound() const function, expected prototype:\nosg::BoundingSpheref osg::LightSource::computeBound() const\nClass arguments details:\n");
		}
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osg::BoundingSpheref osg::LightSource::computeBound() const");
		}
