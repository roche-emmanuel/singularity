...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osg::NodeAcceptOp*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		osg::NodeAcceptOp* self= (osg::NodeAcceptOp*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< osg::NodeAcceptOp >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_asVoid(lua_State *L) {
		...
		void* self= (void*)(Luna< osg::NodeAcceptOp >::check(L,1));
		if(!self) {
...
luna_RegType LunaTraits< osg::NodeAcceptOp >::methods[] = {
	...
	{"fromVoid", &luna_wrapper_osg_NodeAcceptOp::_bind_fromVoid},
	{"asVoid", &luna_wrapper_osg_NodeAcceptOp::_bind_asVoid},
	{0,0}
};
