...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,74764459) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osg::VectorGLuint*)");
		}

		osg::VectorGLuint* rhs =(Luna< osg::VectorGLuint >::check(L,2));
...
luna_RegType LunaTraits< osg::VectorGLuint >::methods[] = {
	...
	{"__eq", &luna_wrapper_osg_VectorGLuint::_bind___eq},
	{0,0}
};
