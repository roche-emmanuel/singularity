...
public:
	...
	static int _bind_base_setThreadSafeRefUnref(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgGA::TerrainManipulator::base_setThreadSafeRefUnref(bool). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	// void osgGA::TerrainManipulator::base_setMinimumDistance(const double & minimumDistance, bool relativeToModelSize = NULL)
	static int _bind_base_setMinimumDistance(lua_State *L) {
		if (!_lg_typecheck_base_setMinimumDistance(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in void osgGA::TerrainManipulator::base_setMinimumDistance(const double & minimumDistance, bool relativeToModelSize = NULL) function, expected prototype:\nvoid osgGA::TerrainManipulator::base_setMinimumDistance(const double & minimumDistance, bool relativeToModelSize = NULL)\nClass arguments details:\n");
		}
