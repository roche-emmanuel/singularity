...
public:
	...
	static int _bind_base_updateCamera(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgGA::TerrainManipulator::base_updateCamera(osg::Camera &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
