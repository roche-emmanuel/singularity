...
public:
	...
	inline static bool _lg_typecheck_remove_overload_1(lua_State *L) {
		...
		if( (lua_isnil(L,2)==0 && !Luna<void>::has_uniqueid(L,2,50169651)) ) return false;
		if( (lua_isnil(L,2)==0 && !dynamic_cast< osg::Operation* >(Luna< osg::Referenced >::check(L,2)) ) ) return false;
		return true;
	}
	...
	static int _bind_runOperations(lua_State *L) {
		...
		osg::Object* callingObject=luatop>1 ? dynamic_cast< osg::Object* >(Luna< osg::Referenced >::check(L,2)) : (osg::Object*)0;
