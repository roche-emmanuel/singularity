...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,9780955) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osg::Vec2fValueObject*)");
		}

		osg::Vec2fValueObject* rhs =(Luna< osg::Vec2fValueObject >::check(L,2));
...
luna_RegType LunaTraits< osg::Vec2fValueObject >::methods[] = {
	...
	{"__eq", &luna_wrapper_osg_Vec2fValueObject::_bind___eq},
	{0,0}
};
