...
public:
	...
	static int _bind_getBufferAttachmentMap_overload_1(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osg::Camera::BufferAttachmentMap & osg::Camera::getBufferAttachmentMap(). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	static int _bind_getBufferAttachmentMap_overload_2(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call const osg::Camera::BufferAttachmentMap & osg::Camera::getBufferAttachmentMap() const. Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
		const osg::Camera::BufferAttachmentMap* lret = &self->getBufferAttachmentMap();
		if(!lret) return 0; // Do not write NULL pointers.
