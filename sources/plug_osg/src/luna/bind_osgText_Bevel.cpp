...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_4(lua_State *L) {
		...
		if( (!dynamic_cast< osgText::Bevel* >(Luna< osg::Referenced >::check(L,2))) ) return false;
	...
	static osgText::Bevel* _bind_ctor_overload_4(lua_State *L) {
		...
		const osgText::Bevel* bevel_ptr=dynamic_cast< osgText::Bevel* >(Luna< osg::Referenced >::check(L,2));
		if( !bevel_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg bevel in osgText::Bevel::Bevel function");
		}
		const osgText::Bevel & bevel=*bevel_ptr;
