...
public:
	...
	static int _bind_load(lua_State *L) {
		...
		const osg::Texture3D* texture_ptr=dynamic_cast< osg::Texture3D* >(Luna< osg::Referenced >::check(L,2));
		if( !texture_ptr ) {
		...
		osg::Texture3D::SubloadCallback* self=dynamic_cast< osg::Texture3D::SubloadCallback* >(Luna< osg::Referenced >::check(L,1));
		if(!self) {
			luna_printStack(L);
