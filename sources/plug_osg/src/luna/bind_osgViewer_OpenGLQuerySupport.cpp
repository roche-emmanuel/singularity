...
public:
	...
	static int _bind_checkQuery(lua_State *L) {
		...
		osg::State* state=dynamic_cast< osg::State* >(Luna< osg::Referenced >::check(L,3));
		unsigned long long startTick=(unsigned long long)lua_tointeger(L,4);

		osgViewer::OpenGLQuerySupport* self=dynamic_cast< osgViewer::OpenGLQuerySupport* >(Luna< osg::Referenced >::check(L,1));
		if(!self) {
			luna_printStack(L);
