...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_2(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,50169651) ) return false;
		if( (!dynamic_cast< osg::OccluderNode* >(Luna< osg::Referenced >::check(L,1))) ) return false;
	...
	static osg::OccluderNode* _bind_ctor_overload_4(lua_State *L) {
		...
		const osg::OccluderNode* _arg2_ptr=dynamic_cast< osg::OccluderNode* >(Luna< osg::Referenced >::check(L,2));
		if( !_arg2_ptr ) {
