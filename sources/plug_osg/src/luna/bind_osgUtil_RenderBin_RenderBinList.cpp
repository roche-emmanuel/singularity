...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,38528519) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osgUtil::RenderBin::RenderBinList*)");
		}

		osgUtil::RenderBin::RenderBinList* rhs =(Luna< osgUtil::RenderBin::RenderBinList >::check(L,2));
...
luna_RegType LunaTraits< osgUtil::RenderBin::RenderBinList >::methods[] = {
	...
	{"__eq", &luna_wrapper_osgUtil_RenderBin_RenderBinList::_bind___eq},
	{0,0}
};
