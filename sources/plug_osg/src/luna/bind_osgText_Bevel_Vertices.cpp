...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,78061823) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osgText::Bevel::Vertices*)");
		}

		osgText::Bevel::Vertices* rhs =(Luna< osgText::Bevel::Vertices >::check(L,2));
...
luna_RegType LunaTraits< osgText::Bevel::Vertices >::methods[] = {
	...
	{"__eq", &luna_wrapper_osgText_Bevel_Vertices::_bind___eq},
	{0,0}
};
