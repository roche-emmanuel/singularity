...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,37099841) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osg::IntValueObject*)");
		}

		osg::IntValueObject* rhs =(Luna< osg::IntValueObject >::check(L,2));
...
luna_RegType LunaTraits< osg::IntValueObject >::methods[] = {
	...
	{"__eq", &luna_wrapper_osg_IntValueObject::_bind___eq},
	{0,0}
};
