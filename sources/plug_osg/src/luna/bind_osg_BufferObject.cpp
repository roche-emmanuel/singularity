...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_2(lua_State *L) {
		...
		if( (!dynamic_cast< osg::BufferObject* >(Luna< osg::Referenced >::check(L,2))) ) return false;
	...
	inline static bool _lg_typecheck_removeBufferData_overload_2(lua_State *L) {
		...
		if( (lua_isnil(L,2)==0 && !Luna<void>::has_uniqueid(L,2,50169651)) ) return false;
		if( (lua_isnil(L,2)==0 && !dynamic_cast< osg::BufferData* >(Luna< osg::Referenced >::check(L,2)) ) ) return false;
		return true;
	}
