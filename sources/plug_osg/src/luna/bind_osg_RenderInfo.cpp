...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_2(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,2286263) ) return false;
		if( (!dynamic_cast< osg::RenderInfo* >(Luna< osg::RenderInfo >::check(L,1))) ) return false;
		return true;
	}
	...
	inline static bool _lg_typecheck_ctor_overload_3(lua_State *L) {
		...
		if( (lua_isnil(L,1)==0 && !dynamic_cast< osg::State* >(Luna< osg::Referenced >::check(L,1)) ) ) return false;
		if( (lua_isnil(L,2)==0 && !Luna<void>::has_uniqueid(L,2,50169651)) ) return false;
		if( (lua_isnil(L,2)==0 && !dynamic_cast< osg::View* >(Luna< osg::Referenced >::check(L,2)) ) ) return false;
		return true;
	}
