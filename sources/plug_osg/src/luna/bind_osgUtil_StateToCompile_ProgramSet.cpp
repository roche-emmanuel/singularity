...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,13242464) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osgUtil::StateToCompile::ProgramSet*)");
		}

		osgUtil::StateToCompile::ProgramSet* rhs =(Luna< osgUtil::StateToCompile::ProgramSet >::check(L,2));
...
luna_RegType LunaTraits< osgUtil::StateToCompile::ProgramSet >::methods[] = {
	...
	{"__eq", &luna_wrapper_osgUtil_StateToCompile_ProgramSet::_bind___eq},
	{0,0}
};
