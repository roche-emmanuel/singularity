...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(GLUtesselator*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		GLUtesselator* self= (GLUtesselator*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< GLUtesselator >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_asVoid(lua_State *L) {
		...
		void* self= (void*)(Luna< GLUtesselator >::check(L,1));
		if(!self) {
...
luna_RegType LunaTraits< GLUtesselator >::methods[] = {
	...
	{"fromVoid", &luna_wrapper_GLUtesselator::_bind_fromVoid},
	{"asVoid", &luna_wrapper_GLUtesselator::_bind_asVoid},
	{0,0}
};
