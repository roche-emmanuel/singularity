...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_2(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,50169651) ) return false;
		if( (!dynamic_cast< osgParticle::BoxPlacer* >(Luna< osg::Referenced >::check(L,1))) ) return false;
	...
	inline static bool _lg_typecheck_ctor_overload_4(lua_State *L) {
		...
		if( (!dynamic_cast< osgParticle::BoxPlacer* >(Luna< osg::Referenced >::check(L,2))) ) return false;
	...
	static osgParticle::BoxPlacer* _bind_ctor_overload_4(lua_State *L) {
		...
		const osgParticle::BoxPlacer* copy_ptr=dynamic_cast< osgParticle::BoxPlacer* >(Luna< osg::Referenced >::check(L,2));
		if( !copy_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg copy in osgParticle::BoxPlacer::BoxPlacer function");
		}
		const osgParticle::BoxPlacer & copy=*copy_ptr;
...
luna_RegType LunaTraits< osgParticle::BoxPlacer >::methods[] = {
	...
	{"__eq", &luna_wrapper_osgParticle_BoxPlacer::_bind___eq},
	{0,0}
};
