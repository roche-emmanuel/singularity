...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,75164642) ) return false;
		return true;
	}
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osgDB::ReaderWriterInfoList*)");
		}

		osgDB::ReaderWriterInfoList* rhs =(Luna< osgDB::ReaderWriterInfoList >::check(L,2));
...
luna_RegType LunaTraits< osgDB::ReaderWriterInfoList >::methods[] = {
	...
	{"__eq", &luna_wrapper_osgDB_ReaderWriterInfoList::_bind___eq},
	{0,0}
};
