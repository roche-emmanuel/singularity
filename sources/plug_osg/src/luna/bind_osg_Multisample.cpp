...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_2(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,50169651) ) return false;
		if( (!dynamic_cast< osg::Multisample* >(Luna< osg::Referenced >::check(L,1))) ) return false;
	...
	inline static bool _lg_typecheck_ctor_overload_4(lua_State *L) {
		...
		if( (!dynamic_cast< osg::Multisample* >(Luna< osg::Referenced >::check(L,2))) ) return false;
	...
	static osg::Multisample* _bind_ctor_overload_4(lua_State *L) {
		...
		const osg::Multisample* trans_ptr=dynamic_cast< osg::Multisample* >(Luna< osg::Referenced >::check(L,2));
		if( !trans_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg trans in osg::Multisample::Multisample function");
		}
		const osg::Multisample & trans=*trans_ptr;
