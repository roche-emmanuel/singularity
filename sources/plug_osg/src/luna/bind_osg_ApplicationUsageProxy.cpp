...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,50027641) ) return false;
		return true;
	}
	
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(osg::ApplicationUsageProxy*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}

		osg::ApplicationUsageProxy* rhs =(Luna< osg::ApplicationUsageProxy >::check(L,2));
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		osg::ApplicationUsageProxy* self= (osg::ApplicationUsageProxy*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< osg::ApplicationUsageProxy >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_asVoid(lua_State *L) {
		...
		void* self= (void*)(Luna< osg::ApplicationUsageProxy >::check(L,1));
		if(!self) {
...
luna_RegType LunaTraits< osg::ApplicationUsageProxy >::methods[] = {
	...
	{"__eq", &luna_wrapper_osg_ApplicationUsageProxy::_bind___eq},
	{"fromVoid", &luna_wrapper_osg_ApplicationUsageProxy::_bind_fromVoid},
	{"asVoid", &luna_wrapper_osg_ApplicationUsageProxy::_bind_asVoid},
	{0,0}
};
