#ifndef _WRAPPERS_WRAPPER_OSG_SHADEMODEL_H_
#define _WRAPPERS_WRAPPER_OSG_SHADEMODEL_H_
...
public:
	...
	~wrapper_osg_ShadeModel() {
		logDEBUG3("Calling delete function for wrapper osg_ShadeModel");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ShadeModel*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ShadeModel(lua_State* L, lua_Table* dum, osg::ShadeModel::Mode mode = osg::ShadeModel::SMOOTH) 
		: osg::ShadeModel(mode), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShadeModel*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ShadeModel(lua_State* L, lua_Table* dum, const osg::ShadeModel & sm, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ShadeModel(sm, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShadeModel*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ShadeModel::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_ShadeModel* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ShadeModel >(L,1);
		if(!self) {
			luna_printStack(L);
