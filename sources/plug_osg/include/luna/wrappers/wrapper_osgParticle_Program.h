...
public:
	...
	~wrapper_osgParticle_Program() {
		logDEBUG3("Calling delete function for wrapper osgParticle_Program");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgParticle::Program*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgParticle_Program(lua_State* L, lua_Table* dum) 
		: osgParticle::Program(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::Program*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgParticle_Program(lua_State* L, lua_Table* dum, const osgParticle::Program & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgParticle::Program(copy, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::Program*)this);
			_obj.callFunction<void>();
...
public:
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call osgParticle::Program & osgParticle::Program::public_op_assign(const osgParticle::Program &). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
