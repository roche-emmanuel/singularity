...
public:
	...
	~wrapper_osg_ConvexHull() {
		logDEBUG3("Calling delete function for wrapper osg_ConvexHull");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ConvexHull*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ConvexHull(lua_State* L, lua_Table* dum) 
		: osg::ConvexHull(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ConvexHull*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ConvexHull(lua_State* L, lua_Table* dum, const osg::ConvexHull & hull, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ConvexHull(hull, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ConvexHull*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ConvexHull::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_ConvexHull* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ConvexHull >(L,1);
		if(!self) {
			luna_printStack(L);
