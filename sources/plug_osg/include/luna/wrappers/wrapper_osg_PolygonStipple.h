...
public:
	...
	~wrapper_osg_PolygonStipple() {
		logDEBUG3("Calling delete function for wrapper osg_PolygonStipple");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::PolygonStipple*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_PolygonStipple(lua_State* L, lua_Table* dum) 
		: osg::PolygonStipple(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::PolygonStipple*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_PolygonStipple(lua_State* L, lua_Table* dum, const unsigned char * mask) 
		: osg::PolygonStipple(mask), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::PolygonStipple*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_PolygonStipple(lua_State* L, lua_Table* dum, const osg::PolygonStipple & lw, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::PolygonStipple(lw, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::PolygonStipple*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return PolygonStipple::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_PolygonStipple* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_PolygonStipple >(L,1);
		if(!self) {
			luna_printStack(L);
