...
public:
	...
	~wrapper_osg_TexEnv() {
		logDEBUG3("Calling delete function for wrapper osg_TexEnv");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::TexEnv*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_TexEnv(lua_State* L, lua_Table* dum, osg::TexEnv::Mode mode = osg::TexEnv::MODULATE) 
		: osg::TexEnv(mode), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexEnv*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_TexEnv(lua_State* L, lua_Table* dum, const osg::TexEnv & texenv, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::TexEnv(texenv, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexEnv*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return TexEnv::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_TexEnv* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_TexEnv >(L,1);
		if(!self) {
			luna_printStack(L);
