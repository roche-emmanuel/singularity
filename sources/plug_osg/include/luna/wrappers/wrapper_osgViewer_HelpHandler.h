...
public:
	...
	~wrapper_osgViewer_HelpHandler() {
		logDEBUG3("Calling delete function for wrapper osgViewer_HelpHandler");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgViewer::HelpHandler*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgViewer_HelpHandler(lua_State* L, lua_Table* dum, osg::ApplicationUsage * au = 0) 
		: osgViewer::HelpHandler(au), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgViewer::HelpHandler*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return HelpHandler::setThreadSafeRefUnref(threadSafe);
	};
