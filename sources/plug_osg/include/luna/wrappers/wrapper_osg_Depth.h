...
public:
	...
	~wrapper_osg_Depth() {
		logDEBUG3("Calling delete function for wrapper osg_Depth");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Depth*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Depth(lua_State* L, lua_Table* dum, osg::Depth::Function func = osg::Depth::LESS, double zNear = 0.0, double zFar = 1.0, bool writeMask = true) 
		: osg::Depth(func, zNear, zFar, writeMask), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Depth*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Depth(lua_State* L, lua_Table* dum, const osg::Depth & dp, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Depth(dp, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Depth*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Depth::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_Depth* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Depth >(L,1);
		if(!self) {
			luna_printStack(L);
