...
public:
	...
	// void osgViewer::Renderer::public_initialize(osg::State * state)
	static int _bind_public_initialize(lua_State *L) {
		if (!_lg_typecheck_public_initialize(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in void osgViewer::Renderer::public_initialize(osg::State * state) function, expected prototype:\nvoid osgViewer::Renderer::public_initialize(osg::State * state)\nClass arguments details:\narg 1 ID = 50169651\n");
		}

		osg::State* state=(Luna< osg::Referenced >::checkSubType< osg::State >(L,2));

		wrapper_osgViewer_Renderer* self=Luna< osg::Referenced >::checkSubType< wrapper_osgViewer_Renderer >(L,1);
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgViewer::Renderer::public_initialize(osg::State *). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
		self->public_initialize(state);
