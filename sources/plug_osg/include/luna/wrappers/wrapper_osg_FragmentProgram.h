...
public:
	...
	~wrapper_osg_FragmentProgram() {
		logDEBUG3("Calling delete function for wrapper osg_FragmentProgram");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::FragmentProgram*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_FragmentProgram(lua_State* L, lua_Table* dum) 
		: osg::FragmentProgram(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::FragmentProgram*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_FragmentProgram(lua_State* L, lua_Table* dum, const osg::FragmentProgram & vp, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::FragmentProgram(vp, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::FragmentProgram*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return FragmentProgram::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_FragmentProgram* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_FragmentProgram >(L,1);
		if(!self) {
			luna_printStack(L);
