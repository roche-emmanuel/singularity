...
public:
	...
	~wrapper_osgViewer_DepthPartitionSettings() {
		logDEBUG3("Calling delete function for wrapper osgViewer_DepthPartitionSettings");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgViewer::DepthPartitionSettings*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgViewer_DepthPartitionSettings(lua_State* L, lua_Table* dum, osgViewer::DepthPartitionSettings::DepthMode mode = osgViewer::DepthPartitionSettings::BOUNDING_VOLUME) 
		: osgViewer::DepthPartitionSettings(mode), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgViewer::DepthPartitionSettings*)this);
			_obj.callFunction<void>();
		}
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osgViewer_DepthPartitionSettings* self=Luna< osg::Referenced >::checkSubType< wrapper_osgViewer_DepthPartitionSettings >(L,1);
		if(!self) {
			luna_printStack(L);
