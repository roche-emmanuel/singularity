...
public:
	...
	~wrapper_osg_Hint() {
		logDEBUG3("Calling delete function for wrapper osg_Hint");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Hint*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Hint(lua_State* L, lua_Table* dum) 
		: osg::Hint(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Hint*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Hint(lua_State* L, lua_Table* dum, unsigned int target, unsigned int mode) 
		: osg::Hint(target, mode), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Hint*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_Hint(lua_State* L, lua_Table* dum, const osg::Hint & hint, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Hint(hint, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Hint*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Hint::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_Hint* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Hint >(L,1);
		if(!self) {
			luna_printStack(L);
