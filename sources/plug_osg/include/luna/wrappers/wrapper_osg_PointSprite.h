#ifndef _WRAPPERS_WRAPPER_OSG_POINTSPRITE_H_
#define _WRAPPERS_WRAPPER_OSG_POINTSPRITE_H_
...
public:
	...
	~wrapper_osg_PointSprite() {
		logDEBUG3("Calling delete function for wrapper osg_PointSprite");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::PointSprite*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_PointSprite(lua_State* L, lua_Table* dum) 
		: osg::PointSprite(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::PointSprite*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_PointSprite(lua_State* L, lua_Table* dum, const osg::PointSprite & ps, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::PointSprite(ps, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::PointSprite*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return PointSprite::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_PointSprite* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_PointSprite >(L,1);
		if(!self) {
			luna_printStack(L);
