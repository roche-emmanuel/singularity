...
public:
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osgParticle::AngularAccelOperator & osgParticle::AngularAccelOperator::public_op_assign(const osgParticle::AngularAccelOperator &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
