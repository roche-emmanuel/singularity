...
public:
	...
	~wrapper_osg_LightSource() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::LightSource*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_LightSource(lua_State* L, lua_Table* dum) 
		: osg::LightSource(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::LightSource*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_LightSource(lua_State* L, lua_Table* dum, const osg::LightSource & ls, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::LightSource(ls, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::LightSource*)this);
			_obj.callFunction<void>();
...
public:
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_LightSource* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_LightSource >(L,1);
		if(!self) {
			luna_printStack(L);
