...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return CenteredPlacer::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		wrapper_osgParticle_CenteredPlacer* self=Luna< osg::Referenced >::checkSubType< wrapper_osgParticle_CenteredPlacer >(L,1);
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osgParticle::Placer & osgParticle::Placer::public_op_assign(const osgParticle::Placer &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
