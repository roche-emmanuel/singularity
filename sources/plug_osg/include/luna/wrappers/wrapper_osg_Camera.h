...
public:
	...
	~wrapper_osg_Camera() {
		logDEBUG3("Calling delete function for wrapper osg_Camera");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Camera*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Camera(lua_State* L, lua_Table* dum) 
		: osg::Camera(), luna_wrapper_base(L) { 
	...
	wrapper_osg_Camera(lua_State* L, lua_Table* dum, const osg::Camera & arg1, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Camera(arg1, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Camera*)this);
			_obj.callFunction<void>();
...
public:
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_Camera* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Camera >(L,1);
		if(!self) {
