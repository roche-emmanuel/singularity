...
public:
	...
	~wrapper_osg_CompositeShape() {
		logDEBUG3("Calling delete function for wrapper osg_CompositeShape");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::CompositeShape*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_CompositeShape(lua_State* L, lua_Table* dum) 
		: osg::CompositeShape(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::CompositeShape*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_CompositeShape(lua_State* L, lua_Table* dum, const osg::CompositeShape & cs, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::CompositeShape(cs, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::CompositeShape*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return CompositeShape::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_CompositeShape* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_CompositeShape >(L,1);
		if(!self) {
			luna_printStack(L);
