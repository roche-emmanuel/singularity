...
public:
	...
	~wrapper_osg_ShaderBinary() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ShaderBinary*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ShaderBinary(lua_State* L, lua_Table* dum) 
		: osg::ShaderBinary(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderBinary*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ShaderBinary(lua_State* L, lua_Table* dum, const osg::ShaderBinary & rhs, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ShaderBinary(rhs, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderBinary*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ShaderBinary::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_ShaderBinary* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ShaderBinary >(L,1);
		if(!self) {
			luna_printStack(L);
