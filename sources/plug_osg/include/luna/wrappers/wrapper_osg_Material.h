#ifndef _WRAPPERS_WRAPPER_OSG_MATERIAL_H_
#define _WRAPPERS_WRAPPER_OSG_MATERIAL_H_
...
public:
	...
	~wrapper_osg_Material() {
		logDEBUG3("Calling delete function for wrapper osg_Material");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Material*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Material(lua_State* L, lua_Table* dum) 
		: osg::Material(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Material*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Material(lua_State* L, lua_Table* dum, const osg::Material & mat, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Material(mat, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Material*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Material::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_Material* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Material >(L,1);
		if(!self) {
			luna_printStack(L);
