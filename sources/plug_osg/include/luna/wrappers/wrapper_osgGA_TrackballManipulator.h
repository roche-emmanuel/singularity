...
public:
	...
	~wrapper_osgGA_TrackballManipulator() {
		logDEBUG3("Calling delete function for wrapper osgGA_TrackballManipulator");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgGA::TrackballManipulator*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgGA_TrackballManipulator(lua_State* L, lua_Table* dum, int flags = osgGA::StandardManipulator::DEFAULT_SETTINGS) 
		: osgGA::TrackballManipulator(flags), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgGA::TrackballManipulator*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgGA_TrackballManipulator(lua_State* L, lua_Table* dum, const osgGA::TrackballManipulator & tm, const osg::CopyOp & copyOp = osg::CopyOp::SHALLOW_COPY) 
		: osgGA::TrackballManipulator(tm, copyOp), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgGA::TrackballManipulator*)this);
			_obj.callFunction<void>();
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return TrackballManipulator::setThreadSafeRefUnref(threadSafe);
	};
	...
	void updateCamera(osg::Camera & camera) {
		...
		return TrackballManipulator::updateCamera(camera);
	};
