...
public:
	...
	~wrapper_osgViewer_ScreenCaptureHandler() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgViewer::ScreenCaptureHandler*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgViewer_ScreenCaptureHandler(lua_State* L, lua_Table* dum, osgViewer::ScreenCaptureHandler::CaptureOperation * defaultOperation = 0, int numFrames = 1) 
		: osgViewer::ScreenCaptureHandler(defaultOperation, numFrames), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgViewer::ScreenCaptureHandler*)this);
			_obj.callFunction<void>();
		}
	};
