...
public:
	...
	~wrapper_osg_AnimationPath() {
		logDEBUG3("Calling delete function for wrapper osg_AnimationPath");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::AnimationPath*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_AnimationPath(lua_State* L, lua_Table* dum) 
		: osg::AnimationPath(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::AnimationPath*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_AnimationPath(lua_State* L, lua_Table* dum, const osg::AnimationPath & ap, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::AnimationPath(ap, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::AnimationPath*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return AnimationPath::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_AnimationPath* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_AnimationPath >(L,1);
		if(!self) {
			luna_printStack(L);
