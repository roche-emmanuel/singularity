#ifndef _WRAPPERS_WRAPPER_OSG_TEXGEN_H_
#define _WRAPPERS_WRAPPER_OSG_TEXGEN_H_
...
public:
	...
	~wrapper_osg_TexGen() {
		logDEBUG3("Calling delete function for wrapper osg_TexGen");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::TexGen*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_TexGen(lua_State* L, lua_Table* dum) 
		: osg::TexGen(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexGen*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_TexGen(lua_State* L, lua_Table* dum, const osg::TexGen & texgen, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::TexGen(texgen, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexGen*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return TexGen::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_TexGen* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_TexGen >(L,1);
		if(!self) {
			luna_printStack(L);
