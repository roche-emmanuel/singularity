...
public:
	...
	~wrapper_osg_OccluderNode() {
		logDEBUG3("Calling delete function for wrapper osg_OccluderNode");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::OccluderNode*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_OccluderNode(lua_State* L, lua_Table* dum) 
		: osg::OccluderNode(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::OccluderNode*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_OccluderNode(lua_State* L, lua_Table* dum, const osg::OccluderNode & arg1, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::OccluderNode(arg1, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::OccluderNode*)this);
			_obj.callFunction<void>();
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return OccluderNode::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_OccluderNode* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_OccluderNode >(L,1);
		if(!self) {
