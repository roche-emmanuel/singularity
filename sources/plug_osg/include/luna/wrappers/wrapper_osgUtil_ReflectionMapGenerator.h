...
public:
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call osgUtil::ReflectionMapGenerator & osgUtil::ReflectionMapGenerator::public_op_assign(const osgUtil::ReflectionMapGenerator &). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
