...
public:
	...
	wrapper_osgParticle_ExplosionOperator(lua_State* L, lua_Table* dum) : osgParticle::ExplosionOperator(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_osgParticle_ExplosionOperator(lua_State* L, lua_Table* dum, const osgParticle::ExplosionOperator & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) : osgParticle::ExplosionOperator(copy, copyop), luna_wrapper_base(L) { register_protected_methods(L); };
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osgParticle::ExplosionOperator & osgParticle::ExplosionOperator::public_op_assign(const osgParticle::ExplosionOperator &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
