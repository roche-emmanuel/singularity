...
public:
	...
	~wrapper_osgViewer_LODScaleHandler() {
		logDEBUG3("Calling delete function for wrapper osgViewer_LODScaleHandler");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgViewer::LODScaleHandler*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgViewer_LODScaleHandler(lua_State* L, lua_Table* dum) 
		: osgViewer::LODScaleHandler(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgViewer::LODScaleHandler*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return LODScaleHandler::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osgViewer_LODScaleHandler* self=Luna< osg::Referenced >::checkSubType< wrapper_osgViewer_LODScaleHandler >(L,1);
		if(!self) {
			luna_printStack(L);
