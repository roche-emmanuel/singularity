...
public:
	...
	~wrapper_osg_VertexBufferObject() {
		logDEBUG3("Calling delete function for wrapper osg_VertexBufferObject");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::VertexBufferObject*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_VertexBufferObject(lua_State* L, lua_Table* dum) 
		: osg::VertexBufferObject(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::VertexBufferObject*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_VertexBufferObject(lua_State* L, lua_Table* dum, const osg::VertexBufferObject & vbo, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::VertexBufferObject(vbo, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::VertexBufferObject*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return VertexBufferObject::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_VertexBufferObject* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_VertexBufferObject >(L,1);
		if(!self) {
			luna_printStack(L);
