#ifndef _WRAPPERS_WRAPPER_OSG_SHADERATTRIBUTE_H_
#define _WRAPPERS_WRAPPER_OSG_SHADERATTRIBUTE_H_
...
class wrapper_osg_ShaderAttribute : public osg::ShaderAttribute, public luna_wrapper_base {

public:
		

	~wrapper_osg_ShaderAttribute() {
		logDEBUG3("Calling delete function for wrapper osg_ShaderAttribute");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ShaderAttribute*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ShaderAttribute(lua_State* L, lua_Table* dum) 
		: osg::ShaderAttribute(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderAttribute*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ShaderAttribute(lua_State* L, lua_Table* dum, const osg::ShaderAttribute & sa, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ShaderAttribute(sa, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderAttribute*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ShaderAttribute::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_ShaderAttribute* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ShaderAttribute >(L,1);
		if(!self) {
			luna_printStack(L);
