...
public:
	...
	~wrapper_osgParticle_ConstantRateCounter() {
		logDEBUG3("Calling delete function for wrapper osgParticle_ConstantRateCounter");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgParticle::ConstantRateCounter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgParticle_ConstantRateCounter(lua_State* L, lua_Table* dum) 
		: osgParticle::ConstantRateCounter(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::ConstantRateCounter*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgParticle_ConstantRateCounter(lua_State* L, lua_Table* dum, const osgParticle::ConstantRateCounter & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgParticle::ConstantRateCounter(copy, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::ConstantRateCounter*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ConstantRateCounter::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		wrapper_osgParticle_ConstantRateCounter* self=Luna< osg::Referenced >::checkSubType< wrapper_osgParticle_ConstantRateCounter >(L,1);
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osgParticle::Counter & osgParticle::Counter::public_op_assign(const osgParticle::Counter &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
