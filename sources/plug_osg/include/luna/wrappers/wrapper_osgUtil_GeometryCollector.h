...
public:
	...
	~wrapper_osgUtil_GeometryCollector() {
		logDEBUG3("Calling delete function for wrapper osgUtil_GeometryCollector");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgUtil::GeometryCollector*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgUtil_GeometryCollector(lua_State* L, lua_Table* dum, osgUtil::Optimizer * optimizer, osgUtil::Optimizer::OptimizationOptions options) 
		: osgUtil::GeometryCollector(optimizer, options), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgUtil::GeometryCollector*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return GeometryCollector::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osgUtil_GeometryCollector* self=Luna< osg::Referenced >::checkSubType< wrapper_osgUtil_GeometryCollector >(L,1);
		if(!self) {
			luna_printStack(L);
