...
public:
	...
	~wrapper_osg_StandardNotifyHandler() {
		logDEBUG3("Calling delete function for wrapper osg_StandardNotifyHandler");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::StandardNotifyHandler*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_StandardNotifyHandler(lua_State* L, lua_Table* dum) 
		: osg::StandardNotifyHandler(), luna_wrapper_base(L) { 
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return StandardNotifyHandler::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_StandardNotifyHandler* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_StandardNotifyHandler >(L,1);
		if(!self) {
			luna_printStack(L);
