...
public:
	...
	~wrapper_osg_NodeCallback() {
		logDEBUG3("Calling delete function for wrapper osg_NodeCallback");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::NodeCallback*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_NodeCallback(lua_State* L, lua_Table* dum) 
		: osg::NodeCallback(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::NodeCallback*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_NodeCallback(lua_State* L, lua_Table* dum, const osg::NodeCallback & nc, const osg::CopyOp & arg2) 
		: osg::NodeCallback(nc, arg2), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::NodeCallback*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return NodeCallback::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_NodeCallback* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_NodeCallback >(L,1);
		if(!self) {
			luna_printStack(L);
