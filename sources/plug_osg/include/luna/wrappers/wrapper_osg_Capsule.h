...
public:
	...
	~wrapper_osg_Capsule() {
		logDEBUG3("Calling delete function for wrapper osg_Capsule");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Capsule*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Capsule(lua_State* L, lua_Table* dum) 
		: osg::Capsule(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Capsule*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Capsule(lua_State* L, lua_Table* dum, const osg::Vec3f & center, float radius, float height) 
		: osg::Capsule(center, radius, height), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Capsule*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_Capsule(lua_State* L, lua_Table* dum, const osg::Capsule & capsule, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Capsule(capsule, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Capsule*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Capsule::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_Capsule* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Capsule >(L,1);
		if(!self) {
			luna_printStack(L);
