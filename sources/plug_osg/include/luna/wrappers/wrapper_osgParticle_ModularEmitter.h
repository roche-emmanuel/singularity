...
public:
	...
	~wrapper_osgParticle_ModularEmitter() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgParticle::ModularEmitter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgParticle_ModularEmitter(lua_State* L, lua_Table* dum) 
		: osgParticle::ModularEmitter(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::ModularEmitter*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgParticle_ModularEmitter(lua_State* L, lua_Table* dum, const osgParticle::ModularEmitter & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgParticle::ModularEmitter(copy, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::ModularEmitter*)this);
			_obj.callFunction<void>();
...
public:
	...
	osg::Camera * asCamera() {
		...
		return ModularEmitter::asCamera();
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ModularEmitter::setThreadSafeRefUnref(threadSafe);
	};
	...
	// osgParticle::ModularEmitter & osgParticle::ModularEmitter::operator=(const osgParticle::ModularEmitter & arg1)
	osgParticle::ModularEmitter & public_op_assign(const osgParticle::ModularEmitter & arg1) {
		return osgParticle::ModularEmitter::operator=(arg1);
	};
	...
	// osgParticle::ModularEmitter & osgParticle::ModularEmitter::public_op_assign(const osgParticle::ModularEmitter & arg1)
	static int _bind_public_op_assign(lua_State *L) {
		if (!_lg_typecheck_public_op_assign(L)) {
			luna_printStack(L);
			luaL_error(L, "luna typecheck failed in osgParticle::ModularEmitter & osgParticle::ModularEmitter::public_op_assign(const osgParticle::ModularEmitter & arg1) function, expected prototype:\nosgParticle::ModularEmitter & osgParticle::ModularEmitter::public_op_assign(const osgParticle::ModularEmitter & arg1)\nClass arguments details:\narg 1 ID = 50169651\n");
		}

		const osgParticle::ModularEmitter* _arg1_ptr=(Luna< osg::Referenced >::checkSubType< osgParticle::ModularEmitter >(L,2));
		if( !_arg1_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg _arg1 in osgParticle::ModularEmitter::public_op_assign function");
		}
		const osgParticle::ModularEmitter & _arg1=*_arg1_ptr;

		wrapper_osgParticle_ModularEmitter* self=Luna< osg::Referenced >::checkSubType< wrapper_osgParticle_ModularEmitter >(L,1);
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osgParticle::ModularEmitter & osgParticle::ModularEmitter::public_op_assign(const osgParticle::ModularEmitter &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
		const osgParticle::ModularEmitter* lret = &self->public_op_assign(_arg1);
		if(!lret) return 0; // Do not write NULL pointers.

		Luna< osgParticle::ModularEmitter >::push(L,lret,false);
