...
public:
	...
	~wrapper_osg_ElementBufferObject() {
		logDEBUG3("Calling delete function for wrapper osg_ElementBufferObject");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ElementBufferObject*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ElementBufferObject(lua_State* L, lua_Table* dum) 
		: osg::ElementBufferObject(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ElementBufferObject*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ElementBufferObject(lua_State* L, lua_Table* dum, const osg::ElementBufferObject & pbo, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ElementBufferObject(pbo, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ElementBufferObject*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ElementBufferObject::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_ElementBufferObject* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ElementBufferObject >(L,1);
		if(!self) {
			luna_printStack(L);
