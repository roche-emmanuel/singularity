...
public:
	...
	~wrapper_osg_StateAttribute() {
		logDEBUG3("Calling delete function for wrapper osg_StateAttribute");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::StateAttribute*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_StateAttribute(lua_State* L, lua_Table* dum) 
		: osg::StateAttribute(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::StateAttribute*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_StateAttribute(lua_State* L, lua_Table* dum, const osg::StateAttribute & sa, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::StateAttribute(sa, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::StateAttribute*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return StateAttribute::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_StateAttribute* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_StateAttribute >(L,1);
		if(!self) {
			luna_printStack(L);
