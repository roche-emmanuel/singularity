...
public:
	...
	~wrapper_osg_ShaderComposer() {
		logDEBUG3("Calling delete function for wrapper osg_ShaderComposer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ShaderComposer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ShaderComposer(lua_State* L, lua_Table* dum) 
		: osg::ShaderComposer(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderComposer*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ShaderComposer(lua_State* L, lua_Table* dum, const osg::ShaderComposer & sa, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ShaderComposer(sa, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderComposer*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ShaderComposer::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_ShaderComposer* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ShaderComposer >(L,1);
		if(!self) {
			luna_printStack(L);
