...
public:
	...
	~wrapper_osg_Texture2DArray_SubloadCallback() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Texture2DArray::SubloadCallback*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Texture2DArray_SubloadCallback(lua_State* L, lua_Table* dum) 
		: osg::Texture2DArray::SubloadCallback(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2DArray::SubloadCallback*)this);
			_obj.callFunction<void>();
		}
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_Texture2DArray_SubloadCallback* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Texture2DArray_SubloadCallback >(L,1);
		if(!self) {
			luna_printStack(L);
