...
public:
	...
	~wrapper_osg_BlendColor() {
		logDEBUG3("Calling delete function for wrapper osg_BlendColor");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::BlendColor*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_BlendColor(lua_State* L, lua_Table* dum) 
		: osg::BlendColor(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::BlendColor*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_BlendColor(lua_State* L, lua_Table* dum, const osg::Vec4f & constantColor) 
		: osg::BlendColor(constantColor), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::BlendColor*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_BlendColor(lua_State* L, lua_Table* dum, const osg::BlendColor & trans, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::BlendColor(trans, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::BlendColor*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return BlendColor::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_BlendColor* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_BlendColor >(L,1);
		if(!self) {
			luna_printStack(L);
