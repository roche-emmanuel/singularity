...
public:
	...
	~wrapper_osg_RunOperations() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::RunOperations*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_RunOperations(lua_State* L, lua_Table* dum) 
		: osg::RunOperations(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::RunOperations*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return RunOperations::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_RunOperations* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_RunOperations >(L,1);
		if(!self) {
			luna_printStack(L);
