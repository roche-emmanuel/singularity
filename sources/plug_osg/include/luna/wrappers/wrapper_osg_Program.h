...
public:
	...
	~wrapper_osg_Program() {
		logDEBUG3("Calling delete function for wrapper osg_Program");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Program*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Program(lua_State* L, lua_Table* dum) 
		: osg::Program(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Program*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Program(lua_State* L, lua_Table* dum, const osg::Program & rhs, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Program(rhs, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Program*)this);
			_obj.callFunction<void>();
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_Program* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Program >(L,1);
		if(!self) {
			luna_printStack(L);
