...
public:
	...
	~wrapper_osg_BlockAndFlushOperation() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::BlockAndFlushOperation*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_BlockAndFlushOperation(lua_State* L, lua_Table* dum) 
		: osg::BlockAndFlushOperation(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::BlockAndFlushOperation*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return BlockAndFlushOperation::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_BlockAndFlushOperation* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_BlockAndFlushOperation >(L,1);
		if(!self) {
			luna_printStack(L);
