...
public:
	...
	~wrapper_osg_Drawable() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Drawable*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Drawable(lua_State* L, lua_Table* dum) 
		: osg::Drawable(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Drawable*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Drawable(lua_State* L, lua_Table* dum, const osg::Drawable & drawable, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Drawable(drawable, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Drawable*)this);
			_obj.callFunction<void>();
