...
public:
	...
	~wrapper_osgUtil_TriStripVisitor() {
		logDEBUG3("Calling delete function for wrapper osgUtil_TriStripVisitor");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgUtil::TriStripVisitor*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgUtil_TriStripVisitor(lua_State* L, lua_Table* dum, osgUtil::Optimizer * optimizer = 0) 
		: osgUtil::TriStripVisitor(optimizer), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgUtil::TriStripVisitor*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return TriStripVisitor::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osgUtil_TriStripVisitor* self=Luna< osg::Referenced >::checkSubType< wrapper_osgUtil_TriStripVisitor >(L,1);
		if(!self) {
			luna_printStack(L);
