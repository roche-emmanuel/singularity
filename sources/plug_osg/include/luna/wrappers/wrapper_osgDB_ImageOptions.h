...
public:
	...
	~wrapper_osgDB_ImageOptions() {
		logDEBUG3("Calling delete function for wrapper osgDB_ImageOptions");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgDB::ImageOptions*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgDB_ImageOptions(lua_State* L, lua_Table* dum) 
		: osgDB::ImageOptions(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgDB::ImageOptions*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgDB_ImageOptions(lua_State* L, lua_Table* dum, const std::string & str) 
		: osgDB::ImageOptions(str), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgDB::ImageOptions*)this);
			_obj.callFunction<void>();
	...
	wrapper_osgDB_ImageOptions(lua_State* L, lua_Table* dum, const osgDB::ImageOptions & options, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgDB::ImageOptions(options, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgDB::ImageOptions*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ImageOptions::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osgDB_ImageOptions* self=Luna< osg::Referenced >::checkSubType< wrapper_osgDB_ImageOptions >(L,1);
		if(!self) {
			luna_printStack(L);
