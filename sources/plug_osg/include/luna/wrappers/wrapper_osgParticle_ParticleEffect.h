...
public:
	...
	~wrapper_osgParticle_ParticleEffect() {
		logDEBUG3("Calling delete function for wrapper osgParticle_ParticleEffect");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgParticle::ParticleEffect*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgParticle_ParticleEffect(lua_State* L, lua_Table* dum, bool automaticSetup = true) 
		: osgParticle::ParticleEffect(automaticSetup), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::ParticleEffect*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgParticle_ParticleEffect(lua_State* L, lua_Table* dum, const osgParticle::ParticleEffect & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgParticle::ParticleEffect(copy, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::ParticleEffect*)this);
			_obj.callFunction<void>();
...
public:
	...
	osg::Camera * asCamera() {
		...
		return ParticleEffect::asCamera();
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ParticleEffect::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osgParticle_ParticleEffect* self=Luna< osg::Referenced >::checkSubType< wrapper_osgParticle_ParticleEffect >(L,1);
		if(!self) {
			luna_printStack(L);
