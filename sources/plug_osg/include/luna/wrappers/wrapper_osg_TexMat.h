#ifndef _WRAPPERS_WRAPPER_OSG_TEXMAT_H_
#define _WRAPPERS_WRAPPER_OSG_TEXMAT_H_
...
public:
	...
	~wrapper_osg_TexMat() {
		logDEBUG3("Calling delete function for wrapper osg_TexMat");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::TexMat*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_TexMat(lua_State* L, lua_Table* dum) 
		: osg::TexMat(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexMat*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_TexMat(lua_State* L, lua_Table* dum, const osg::Matrixd & matrix) 
		: osg::TexMat(matrix), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexMat*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_TexMat(lua_State* L, lua_Table* dum, const osg::TexMat & texmat, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::TexMat(texmat, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexMat*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return TexMat::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_TexMat* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_TexMat >(L,1);
		if(!self) {
			luna_printStack(L);
