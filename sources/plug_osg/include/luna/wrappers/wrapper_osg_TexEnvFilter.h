...
public:
	...
	~wrapper_osg_TexEnvFilter() {
		logDEBUG3("Calling delete function for wrapper osg_TexEnvFilter");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::TexEnvFilter*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_TexEnvFilter(lua_State* L, lua_Table* dum, float lodBias = 0.0f) 
		: osg::TexEnvFilter(lodBias), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexEnvFilter*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_TexEnvFilter(lua_State* L, lua_Table* dum, const osg::TexEnvFilter & texenv, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::TexEnvFilter(texenv, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::TexEnvFilter*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return TexEnvFilter::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osg_TexEnvFilter* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_TexEnvFilter >(L,1);
		if(!self) {
			luna_printStack(L);
