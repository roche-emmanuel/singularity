...
public:
	...
	~wrapper_osgUtil_SmoothingVisitor() {
		logDEBUG3("Calling delete function for wrapper osgUtil_SmoothingVisitor");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgUtil::SmoothingVisitor*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgUtil_SmoothingVisitor(lua_State* L, lua_Table* dum) 
		: osgUtil::SmoothingVisitor(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgUtil::SmoothingVisitor*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return SmoothingVisitor::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osgUtil_SmoothingVisitor* self=Luna< osg::Referenced >::checkSubType< wrapper_osgUtil_SmoothingVisitor >(L,1);
		if(!self) {
			luna_printStack(L);
