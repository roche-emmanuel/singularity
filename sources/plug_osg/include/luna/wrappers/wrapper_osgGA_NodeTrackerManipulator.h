...
public:
	...
	~wrapper_osgGA_NodeTrackerManipulator() {
		logDEBUG3("Calling delete function for wrapper osgGA_NodeTrackerManipulator");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgGA::NodeTrackerManipulator*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgGA_NodeTrackerManipulator(lua_State* L, lua_Table* dum, int flags = osgGA::StandardManipulator::DEFAULT_SETTINGS) 
		: osgGA::NodeTrackerManipulator(flags), luna_wrapper_base(L) { 
	...
	wrapper_osgGA_NodeTrackerManipulator(lua_State* L, lua_Table* dum, const osgGA::NodeTrackerManipulator & om, const osg::CopyOp & copyOp = osg::CopyOp::SHALLOW_COPY) 
		: osgGA::NodeTrackerManipulator(om, copyOp), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgGA::NodeTrackerManipulator*)this);
			_obj.callFunction<void>();
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return NodeTrackerManipulator::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_computeNodeWorldToLocal(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgGA::NodeTrackerManipulator::public_computeNodeWorldToLocal(osg::Matrixd &) const. Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	static int _bind_public_computeNodeLocalToWorld(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgGA::NodeTrackerManipulator::public_computeNodeLocalToWorld(osg::Matrixd &) const. Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	static int _bind_public_computeNodeCenterAndRotation(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgGA::NodeTrackerManipulator::public_computeNodeCenterAndRotation(osg::Vec3d &, osg::Quat &) const. Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	static int _bind_public_computePosition(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void osgGA::NodeTrackerManipulator::public_computePosition(const osg::Vec3d &, const osg::Vec3d &, const osg::Vec3d &). Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	void register_protected_methods(lua_State* L) {
		...
		{"computeNodeWorldToLocal",_bind_public_computeNodeWorldToLocal},
		{"computeNodeLocalToWorld",_bind_public_computeNodeLocalToWorld},
		{"computeNodeCenterAndRotation",_bind_public_computeNodeCenterAndRotation},
