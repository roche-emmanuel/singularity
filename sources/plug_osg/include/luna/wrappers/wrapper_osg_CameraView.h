...
public:
	...
	~wrapper_osg_CameraView() {
		logDEBUG3("Calling delete function for wrapper osg_CameraView");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::CameraView*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_CameraView(lua_State* L, lua_Table* dum) 
		: osg::CameraView(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::CameraView*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_CameraView(lua_State* L, lua_Table* dum, const osg::CameraView & pat, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::CameraView(pat, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::CameraView*)this);
			_obj.callFunction<void>();
