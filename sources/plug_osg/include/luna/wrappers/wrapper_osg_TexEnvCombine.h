...
public:
	...
	static int _bind_public_needsTexEnvCombiner(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call bool osg::TexEnvCombine::public_needsTexEnvCombiner(int) const. Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_public_computeNeedForTexEnvCombiners(lua_State *L) {
		...
		wrapper_osg_TexEnvCombine* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_TexEnvCombine >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void osg::TexEnvCombine::public_computeNeedForTexEnvCombiners(). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
