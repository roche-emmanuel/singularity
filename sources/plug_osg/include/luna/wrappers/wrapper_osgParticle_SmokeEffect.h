...
public:
	...
	~wrapper_osgParticle_SmokeEffect() {
		logDEBUG3("Calling delete function for wrapper osgParticle_SmokeEffect");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgParticle::SmokeEffect*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgParticle_SmokeEffect(lua_State* L, lua_Table* dum, bool automaticSetup = true) 
		: osgParticle::SmokeEffect(automaticSetup), luna_wrapper_base(L) { 
	...
	wrapper_osgParticle_SmokeEffect(lua_State* L, lua_Table* dum, const osg::Vec3f & position, float scale = 1.0f, float intensity = 1.0f) 
		: osgParticle::SmokeEffect(position, scale, intensity), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::SmokeEffect*)this);
			_obj.callFunction<void>();
	...
	wrapper_osgParticle_SmokeEffect(lua_State* L, lua_Table* dum, const osgParticle::SmokeEffect & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgParticle::SmokeEffect(copy, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::SmokeEffect*)this);
			_obj.callFunction<void>();
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return SmokeEffect::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_addParent(lua_State *L) {
		...
		wrapper_osgParticle_SmokeEffect* self=Luna< osg::Referenced >::checkSubType< wrapper_osgParticle_SmokeEffect >(L,1);
		if(!self) {
			luna_printStack(L);
