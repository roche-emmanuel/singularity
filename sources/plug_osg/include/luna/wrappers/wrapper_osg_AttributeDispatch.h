...
public:
	...
	~wrapper_osg_AttributeDispatch() {
		logDEBUG3("Calling delete function for wrapper osg_AttributeDispatch");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::AttributeDispatch*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_AttributeDispatch(lua_State* L, lua_Table* dum) 
		: osg::AttributeDispatch(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::AttributeDispatch*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return AttributeDispatch::setThreadSafeRefUnref(threadSafe);
	};
