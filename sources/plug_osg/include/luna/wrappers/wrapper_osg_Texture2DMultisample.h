...
public:
	...
	~wrapper_osg_Texture2DMultisample() {
		logDEBUG3("Calling delete function for wrapper osg_Texture2DMultisample");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Texture2DMultisample*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Texture2DMultisample(lua_State* L, lua_Table* dum) 
		: osg::Texture2DMultisample(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2DMultisample*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Texture2DMultisample(lua_State* L, lua_Table* dum, int numSamples, unsigned char fixedsamplelocations) 
		: osg::Texture2DMultisample(numSamples, fixedsamplelocations), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2DMultisample*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_Texture2DMultisample(lua_State* L, lua_Table* dum, const osg::Texture2DMultisample & text, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Texture2DMultisample(text, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2DMultisample*)this);
			_obj.callFunction<void>();
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Texture2DMultisample::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_computeInternalFormatWithImage(lua_State *L) {
		...
		wrapper_osg_Texture2DMultisample* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Texture2DMultisample >(L,1);
		if(!self) {
			luna_printStack(L);
