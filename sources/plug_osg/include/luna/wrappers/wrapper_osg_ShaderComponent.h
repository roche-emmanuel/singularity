...
public:
	...
	~wrapper_osg_ShaderComponent() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::ShaderComponent*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_ShaderComponent(lua_State* L, lua_Table* dum) 
		: osg::ShaderComponent(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderComponent*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_ShaderComponent(lua_State* L, lua_Table* dum, const osg::ShaderComponent & sc, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::ShaderComponent(sc, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::ShaderComponent*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return ShaderComponent::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_ShaderComponent* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_ShaderComponent >(L,1);
		if(!self) {
			luna_printStack(L);
