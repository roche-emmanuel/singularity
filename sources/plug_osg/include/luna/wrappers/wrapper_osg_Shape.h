...
public:
	...
	~wrapper_osg_Shape() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Shape*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Shape(lua_State* L, lua_Table* dum) 
		: osg::Shape(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Shape*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Shape(lua_State* L, lua_Table* dum, const osg::Shape & sa, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Shape(sa, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Shape*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Shape::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_Shape* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Shape >(L,1);
		if(!self) {
			luna_printStack(L);
