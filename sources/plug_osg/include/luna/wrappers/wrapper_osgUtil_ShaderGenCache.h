...
public:
	...
	wrapper_osgUtil_ShaderGenCache(lua_State* L, lua_Table* dum) : osgUtil::ShaderGenCache(), luna_wrapper_base(L) { register_protected_methods(L); };
	...
	static int _bind_public_createStateSet(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call osg::StateSet * osgUtil::ShaderGenCache::public_createStateSet(int) const. Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	void register_protected_methods(lua_State* L) {
		...
		{"protected_createStateSet",_bind_public_createStateSet},
