...
public:
	...
	~wrapper_osg_Sphere() {
		logDEBUG3("Calling delete function for wrapper osg_Sphere");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Sphere*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Sphere(lua_State* L, lua_Table* dum) 
		: osg::Sphere(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Sphere*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Sphere(lua_State* L, lua_Table* dum, const osg::Vec3f & center, float radius) 
		: osg::Sphere(center, radius), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Sphere*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_Sphere(lua_State* L, lua_Table* dum, const osg::Sphere & sphere, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Sphere(sphere, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Sphere*)this);
			_obj.callFunction<void>();
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Sphere::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_signalObserversAndDelete(lua_State *L) {
		...
		wrapper_osg_Sphere* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Sphere >(L,1);
		if(!self) {
			luna_printStack(L);
