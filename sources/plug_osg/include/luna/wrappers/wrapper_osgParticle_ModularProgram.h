...
public:
	...
	static int _bind_public_op_assign(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call osgParticle::ModularProgram & osgParticle::ModularProgram::public_op_assign(const osgParticle::ModularProgram &). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
