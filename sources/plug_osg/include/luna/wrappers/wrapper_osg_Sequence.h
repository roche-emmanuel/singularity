...
public:
	...
	static int _bind_public__getNextValue(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call int osg::Sequence::public__getNextValue(). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_public__update(lua_State *L) {
		...
		wrapper_osg_Sequence* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Sequence >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call void osg::Sequence::public__update(). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->public__update();
