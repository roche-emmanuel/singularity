...
public:
	...
	~wrapper_osgParticle_FireEffect() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osgParticle::FireEffect*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osgParticle_FireEffect(lua_State* L, lua_Table* dum, bool automaticSetup = true) 
		: osgParticle::FireEffect(automaticSetup), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::FireEffect*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osgParticle_FireEffect(lua_State* L, lua_Table* dum, const osg::Vec3f & position, float scale = 1.0f, float intensity = 1.0f) 
		: osgParticle::FireEffect(position, scale, intensity), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::FireEffect*)this);
			_obj.callFunction<void>();
	...
	wrapper_osgParticle_FireEffect(lua_State* L, lua_Table* dum, const osgParticle::FireEffect & copy, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osgParticle::FireEffect(copy, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osgParticle::FireEffect*)this);
			_obj.callFunction<void>();
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return FireEffect::setThreadSafeRefUnref(threadSafe);
	};
