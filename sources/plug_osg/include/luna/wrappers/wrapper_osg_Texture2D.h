...
class wrapper_osg_Texture2D : public osg::Texture2D, public luna_wrapper_base {

public:
		

	~wrapper_osg_Texture2D() {
		logDEBUG3("Calling delete function for wrapper osg_Texture2D");
		if(_obj.pushFunction("delete")) {
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Texture2D(lua_State* L, lua_Table* dum) : osg::Texture2D(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_osg_Texture2D(lua_State* L, lua_Table* dum, osg::Image * image) : osg::Texture2D(image), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_osg_Texture2D(lua_State* L, lua_Table* dum, const osg::Texture2D & text, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) : osg::Texture2D(text, copyop), luna_wrapper_base(L) { register_protected_methods(L); };
...
public:
	...
	void setThreadSafeRefUnref(bool threadSafe) {
		...
		return Texture2D::setThreadSafeRefUnref(threadSafe);
	};
	...
	static int _bind_public_textureObjectValid(lua_State *L) {
		...
		wrapper_osg_Texture2D* self=Luna< osg::Referenced >::checkSubType< wrapper_osg_Texture2D >(L,1);
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call bool osg::Texture2D::public_textureObjectValid(osg::State &) const. Got : '%s'",typeid(Luna< osg::Referenced >::check(L,1)).name());
		}
	...
	void register_protected_methods(lua_State* L) {
		...
		{"protected_textureObjectValid",_bind_public_textureObjectValid},
