...
public:
	...
	~wrapper_osg_Texture2D() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((osg::Texture2D*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_osg_Texture2D(lua_State* L, lua_Table* dum) 
		: osg::Texture2D(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2D*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_osg_Texture2D(lua_State* L, lua_Table* dum, osg::Image * image) 
		: osg::Texture2D(image), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2D*)this);
			_obj.callFunction<void>();
	...
	wrapper_osg_Texture2D(lua_State* L, lua_Table* dum, const osg::Texture2D & text, const osg::CopyOp & copyop = osg::CopyOp::SHALLOW_COPY) 
		: osg::Texture2D(text, copyop), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((osg::Texture2D*)this);
			_obj.callFunction<void>();
