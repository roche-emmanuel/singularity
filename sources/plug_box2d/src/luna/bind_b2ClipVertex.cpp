...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(b2ClipVertex*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_getV(lua_State *L) {
		...
		b2ClipVertex* self=(Luna< b2ClipVertex >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call b2Vec2 b2ClipVertex::v(). Got : '%s'\n%s",typeid(Luna< b2ClipVertex >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_setV(lua_State *L) {
		...
		b2ClipVertex* self=(Luna< b2ClipVertex >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void b2ClipVertex::v(b2Vec2). Got : '%s'\n%s",typeid(Luna< b2ClipVertex >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->v = value;
