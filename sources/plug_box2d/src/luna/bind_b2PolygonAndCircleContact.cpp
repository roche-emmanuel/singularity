...
public:
	...
	static int _bind_Evaluate(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void b2PolygonAndCircleContact::Evaluate(b2Manifold *, const b2Transform &, const b2Transform &). Got : '%s'",typeid(Luna< b2Contact >::check(L,1)).name());
		}
	...
	static int _bind_base_Evaluate(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void b2PolygonAndCircleContact::base_Evaluate(b2Manifold *, const b2Transform &, const b2Transform &). Got : '%s'",typeid(Luna< b2Contact >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< b2PolygonAndCircleContact >::methods[] = {
	...
	{"__eq", &luna_wrapper_b2PolygonAndCircleContact::_bind___eq},
