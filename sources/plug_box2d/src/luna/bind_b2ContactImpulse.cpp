...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(b2ContactImpulse*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		b2ContactImpulse* self= (b2ContactImpulse*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< b2ContactImpulse >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_asVoid(lua_State *L) {
		...
		void* self= (void*)(Luna< b2ContactImpulse >::check(L,1));
		if(!self) {
	...
	static int _bind_getCount(lua_State *L) {
		...
		b2ContactImpulse* self=(Luna< b2ContactImpulse >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call signed int b2ContactImpulse::count(). Got : '%s'\n%s",typeid(Luna< b2ContactImpulse >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_setCount(lua_State *L) {
		...
		b2ContactImpulse* self=(Luna< b2ContactImpulse >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void b2ContactImpulse::count(signed int). Got : '%s'\n%s",typeid(Luna< b2ContactImpulse >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->count = value;
...
luna_RegType LunaTraits< b2ContactImpulse >::methods[] = {
	...
	{"fromVoid", &luna_wrapper_b2ContactImpulse::_bind_fromVoid},
	{"asVoid", &luna_wrapper_b2ContactImpulse::_bind_asVoid},
	{0,0}
};
