...
public:
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(b2QueryCallback*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_ReportFixture(lua_State *L) {
		...
		b2QueryCallback* self=(Luna< b2QueryCallback >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool b2QueryCallback::ReportFixture(b2Fixture *). Got : '%s'\n%s",typeid(Luna< b2QueryCallback >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
