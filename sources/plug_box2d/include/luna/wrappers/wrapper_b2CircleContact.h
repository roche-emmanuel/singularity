#ifndef _WRAPPERS_WRAPPER_B_CIRCLECONTACT_H_
#define _WRAPPERS_WRAPPER_B_CIRCLECONTACT_H_
...
class wrapper_b2CircleContact : public b2CircleContact, public luna_wrapper_base {

public:
		

	~wrapper_b2CircleContact() {
		logDEBUG3("Calling delete function for wrapper b2CircleContact");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2CircleContact*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2CircleContact(lua_State* L, lua_Table* dum, b2Fixture * fixtureA, b2Fixture * fixtureB) 
		: b2CircleContact(fixtureA, fixtureB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2CircleContact*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void Evaluate(b2Manifold * manifold, const b2Transform & xfA, const b2Transform & xfB) {
		...
		return b2CircleContact::Evaluate(manifold, xfA, xfB);
	};
