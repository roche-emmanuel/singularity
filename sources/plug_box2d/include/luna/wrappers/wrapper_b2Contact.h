#ifndef _WRAPPERS_WRAPPER_B_CONTACT_H_
#define _WRAPPERS_WRAPPER_B_CONTACT_H_
...
class wrapper_b2Contact : public b2Contact, public luna_wrapper_base {

public:
		

	~wrapper_b2Contact() {
		logDEBUG3("Calling delete function for wrapper b2Contact");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2Contact*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2Contact(lua_State* L, lua_Table* dum) 
		: b2Contact(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2Contact*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_b2Contact(lua_State* L, lua_Table* dum, b2Fixture * fixtureA, signed int indexA, b2Fixture * fixtureB, signed int indexB) 
		: b2Contact(fixtureA, indexA, fixtureB, indexB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2Contact*)this);
			_obj.callFunction<void>();
	...
	void Evaluate(b2Manifold * manifold, const b2Transform & xfA, const b2Transform & xfB) {
		THROW_IF(!_obj.pushFunction("Evaluate"),"No implementation for abstract function b2Contact::Evaluate");
		_obj.pushArg((b2Contact*)this);
	...
	static int _bind_public_FlagForFiltering(lua_State *L) {
		...
		wrapper_b2Contact* self=Luna< b2Contact >::checkSubType< wrapper_b2Contact >(L,1);
		if(!self) {
