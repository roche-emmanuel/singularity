#ifndef _WRAPPERS_WRAPPER_B_CHAINANDPOLYGONCONTACT_H_
#define _WRAPPERS_WRAPPER_B_CHAINANDPOLYGONCONTACT_H_
...
class wrapper_b2ChainAndPolygonContact : public b2ChainAndPolygonContact, public luna_wrapper_base {

public:
		

	~wrapper_b2ChainAndPolygonContact() {
		logDEBUG3("Calling delete function for wrapper b2ChainAndPolygonContact");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2ChainAndPolygonContact*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2ChainAndPolygonContact(lua_State* L, lua_Table* dum, b2Fixture * fixtureA, signed int indexA, b2Fixture * fixtureB, signed int indexB) 
		: b2ChainAndPolygonContact(fixtureA, indexA, fixtureB, indexB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2ChainAndPolygonContact*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void Evaluate(b2Manifold * manifold, const b2Transform & xfA, const b2Transform & xfB) {
		...
		return b2ChainAndPolygonContact::Evaluate(manifold, xfA, xfB);
	};
	...
	static int _bind_public_FlagForFiltering(lua_State *L) {
		...
		wrapper_b2ChainAndPolygonContact* self=Luna< b2Contact >::checkSubType< wrapper_b2ChainAndPolygonContact >(L,1);
		if(!self) {
			luna_printStack(L);
