#ifndef _WRAPPERS_WRAPPER_B_ROPEJOINT_H_
#define _WRAPPERS_WRAPPER_B_ROPEJOINT_H_
...
class wrapper_b2RopeJoint : public b2RopeJoint, public luna_wrapper_base {

public:
		

	~wrapper_b2RopeJoint() {
		logDEBUG3("Calling delete function for wrapper b2RopeJoint");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2RopeJoint*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2RopeJoint(lua_State* L, lua_Table* dum, const b2RopeJointDef * data) 
		: b2RopeJoint(data), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2RopeJoint*)this);
			_obj.callFunction<void>();
		}
	};
...
protected:
	...
	void InitVelocityConstraints(const b2SolverData & data) {
		...
		return b2RopeJoint::InitVelocityConstraints(data);
	};
	...
	void SolveVelocityConstraints(const b2SolverData & data) {
		...
		return b2RopeJoint::SolveVelocityConstraints(data);
	};
	...
	bool SolvePositionConstraints(const b2SolverData & data) {
		...
		return b2RopeJoint::SolvePositionConstraints(data);
	};
...
public:
	...
	b2Vec2 GetReactionForce(float inv_dt) const {
		...
		return b2RopeJoint::GetReactionForce(inv_dt);
	};
	...
	float GetReactionTorque(float inv_dt) const {
		...
		return b2RopeJoint::GetReactionTorque(inv_dt);
	};
	...
	static int _bind_public_Create(lua_State *L) {
		...
		wrapper_b2RopeJoint* self=Luna< b2Joint >::checkSubType< wrapper_b2RopeJoint >(L,1);
		if(!self) {
			luna_printStack(L);
