#ifndef _WRAPPERS_WRAPPER_B_EDGEANDPOLYGONCONTACT_H_
#define _WRAPPERS_WRAPPER_B_EDGEANDPOLYGONCONTACT_H_
...
class wrapper_b2EdgeAndPolygonContact : public b2EdgeAndPolygonContact, public luna_wrapper_base {

public:
		

	~wrapper_b2EdgeAndPolygonContact() {
		logDEBUG3("Calling delete function for wrapper b2EdgeAndPolygonContact");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2EdgeAndPolygonContact*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2EdgeAndPolygonContact(lua_State* L, lua_Table* dum, b2Fixture * fixtureA, b2Fixture * fixtureB) 
		: b2EdgeAndPolygonContact(fixtureA, fixtureB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2EdgeAndPolygonContact*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void Evaluate(b2Manifold * manifold, const b2Transform & xfA, const b2Transform & xfB) {
		...
		return b2EdgeAndPolygonContact::Evaluate(manifold, xfA, xfB);
	};
	...
	static int _bind_public_FlagForFiltering(lua_State *L) {
		...
		wrapper_b2EdgeAndPolygonContact* self=Luna< b2Contact >::checkSubType< wrapper_b2EdgeAndPolygonContact >(L,1);
		if(!self) {
