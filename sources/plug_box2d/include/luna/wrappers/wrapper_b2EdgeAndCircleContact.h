#ifndef _WRAPPERS_WRAPPER_B_EDGEANDCIRCLECONTACT_H_
#define _WRAPPERS_WRAPPER_B_EDGEANDCIRCLECONTACT_H_
...
class wrapper_b2EdgeAndCircleContact : public b2EdgeAndCircleContact, public luna_wrapper_base {

public:
		

	~wrapper_b2EdgeAndCircleContact() {
		logDEBUG3("Calling delete function for wrapper b2EdgeAndCircleContact");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2EdgeAndCircleContact*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2EdgeAndCircleContact(lua_State* L, lua_Table* dum, b2Fixture * fixtureA, b2Fixture * fixtureB) 
		: b2EdgeAndCircleContact(fixtureA, fixtureB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2EdgeAndCircleContact*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void Evaluate(b2Manifold * manifold, const b2Transform & xfA, const b2Transform & xfB) {
		...
		return b2EdgeAndCircleContact::Evaluate(manifold, xfA, xfB);
	};
	...
	static int _bind_public_FlagForFiltering(lua_State *L) {
		...
		wrapper_b2EdgeAndCircleContact* self=Luna< b2Contact >::checkSubType< wrapper_b2EdgeAndCircleContact >(L,1);
		if(!self) {
