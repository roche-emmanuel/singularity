#ifndef _WRAPPERS_WRAPPER_B_CHAINANDCIRCLECONTACT_H_
#define _WRAPPERS_WRAPPER_B_CHAINANDCIRCLECONTACT_H_
...
class wrapper_b2ChainAndCircleContact : public b2ChainAndCircleContact, public luna_wrapper_base {

public:
		

	~wrapper_b2ChainAndCircleContact() {
		logDEBUG3("Calling delete function for wrapper b2ChainAndCircleContact");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((b2ChainAndCircleContact*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_b2ChainAndCircleContact(lua_State* L, lua_Table* dum, b2Fixture * fixtureA, signed int indexA, b2Fixture * fixtureB, signed int indexB) 
		: b2ChainAndCircleContact(fixtureA, indexA, fixtureB, indexB), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((b2ChainAndCircleContact*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void Evaluate(b2Manifold * manifold, const b2Transform & xfA, const b2Transform & xfB) {
		...
		return b2ChainAndCircleContact::Evaluate(manifold, xfA, xfB);
	};
	...
	static int _bind_public_FlagForFiltering(lua_State *L) {
		...
		wrapper_b2ChainAndCircleContact* self=Luna< b2Contact >::checkSubType< wrapper_b2ChainAndCircleContact >(L,1);
		if(!self) {
			luna_printStack(L);
