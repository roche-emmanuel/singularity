...
int MicroPather::SolveForNearStates( void* startState, std::vector< StateCost >* near2, float maxCost )
{
...
#ifdef DEBUG
	for( unsigned i=0; i<near2->size(); ++i ) {
		for( unsigned k=i+1; k<near2->size(); ++k ) {
			MPASSERT( near2->at(i).state != near2->at(k).state );
		}
	}
#endif
