	  ...
	  public:
		...
		*/
		...
		/**
			...
			@param startState	Input, the starting state for the path.
			@param nears		All the states within 'maxCost' of 'startState', and cost to that state.
			@param maxCost		Input, the maximum cost that will be returned. (Higher values return
								larger 'nears' sets and take more time to compute.)
			@return				Success or failure, expressed as SOLVED or NO_SOLUTION.
		*/
		int SolveForNearStates( void* startState, std::vector< StateCost >* nears, float maxCost );
