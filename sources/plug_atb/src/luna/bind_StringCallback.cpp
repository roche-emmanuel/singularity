...
public:
	...
	static int _bind_setValue(lua_State *L) {
		...
		if(!self) {
			luaL_error(L, "Invalid object in function call void StringCallback::setValue(const std::string &). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->setValue(val);
	...
	static int _bind_getValue(lua_State *L) {
		...
		StringCallback* self=Luna< osg::Referenced >::checkSubType< StringCallback >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call std::string StringCallback::getValue(). Got : '%s'\n%s",typeid(Luna< osg::Referenced >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
