...
public:
	...
	inline static bool _lg_typecheck___eq(lua_State *L) {
		...
		if( !Luna<void>::has_uniqueid(L,1,82714739) ) return false;
		return true;
	}
	
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(SDL_sem*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}

		SDL_sem* rhs =(Luna< SDL_sem >::check(L,2));
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		SDL_sem* self= (SDL_sem*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< SDL_sem >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_asVoid(lua_State *L) {
		...
		void* self= (void*)(Luna< SDL_sem >::check(L,1));
		if(!self) {
...
const int LunaTraits< SDL_sem >::hash = 82714739;
...
luna_RegType LunaTraits< SDL_sem >::methods[] = {
	...
	{"__eq", &luna_wrapper_SDL_sem::_bind___eq},
	{"fromVoid", &luna_wrapper_SDL_sem::_bind_fromVoid},
	{"asVoid", &luna_wrapper_SDL_sem::_bind_asVoid},
	{0,0}
};
