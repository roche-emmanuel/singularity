...
public:
	...
	static int _bind_base_Clone(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call wxEvent * wxListEvent::base_Clone() const. Got : '%s'",typeid(Luna< wxObject >::check(L,1)).name());
		}
...
luna_RegType LunaTraits< wxListEvent >::methods[] = {
	...
	{"__eq", &luna_wrapper_wxListEvent::_bind___eq},
