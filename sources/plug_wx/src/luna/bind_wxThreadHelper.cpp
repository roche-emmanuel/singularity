...
public:
	...
	static int _bind_getTable(lua_State *L) {
		...
		luna_wrapper_base* wrapper = luna_caster<wxThreadHelper,luna_wrapper_base>::cast(self); //dynamic_cast<luna_wrapper_base*>(self);
		if(wrapper) {
	...
	static int _bind___eq(lua_State *L) {
		if (!_lg_typecheck___eq(L)) {
			luaL_error(L, "luna typecheck failed in __eq function, expected prototype:\n__eq(wxThreadHelper*). Got arguments:\n%s",luna_dumpStack(L).c_str());
		}
	...
	static int _bind_fromVoid(lua_State *L) {
		...
		wxThreadHelper* self= (wxThreadHelper*)(Luna< void >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call fromVoid(...)");
		}
		
		Luna< wxThreadHelper >::push(L,self,false);
		return 1;
	}
	...
	static int _bind_asVoid(lua_State *L) {
		...
		void* self= (void*)(Luna< wxThreadHelper >::check(L,1));
		if(!self) {
...
luna_RegType LunaTraits< wxThreadHelper >::methods[] = {
	...
	{"fromVoid", &luna_wrapper_wxThreadHelper::_bind_fromVoid},
	{"asVoid", &luna_wrapper_wxThreadHelper::_bind_asVoid},
