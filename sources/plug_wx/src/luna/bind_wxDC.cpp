...
public:
	...
	inline static bool _lg_typecheck_DrawLabel_overload_1(lua_State *L) {
		...
		if( (!dynamic_cast< wxRect* >(Luna< wxRect >::check(L,4))) ) return false;
		if( luatop>4 && (lua_isnumber(L,5)==0 || lua_tointeger(L,5) != lua_tonumber(L,5)) ) return false;
		if( luatop>5 && (lua_isnumber(L,6)==0 || lua_tointeger(L,6) != lua_tonumber(L,6)) ) return false;
		if( luatop>6 && (lua_isnil(L,7)==0 && !Luna<void>::has_uniqueid(L,7,20234418)) ) return false;
		if( luatop>6 && (lua_isnil(L,7)==0 && !dynamic_cast< wxRect* >(Luna< wxRect >::check(L,7)) ) ) return false;
		return true;
	}
	...
	inline static bool _lg_typecheck_DrawLabel_overload_2(lua_State *L) {
		...
		if( (!dynamic_cast< wxRect* >(Luna< wxRect >::check(L,3))) ) return false;
	...
	inline static bool _lg_typecheck_GetMultiLineTextExtent_overload_1(lua_State *L) {
		...
		if( luatop>5 && (lua_isnil(L,6)==0 && !dynamic_cast< wxFont* >(Luna< wxObject >::check(L,6)) ) ) return false;
		return true;
	}
