...
public:
	...
	// HWND wxHtmlHelpFrame::base_GetHandle() const
	static int _bind_base_GetHandle(lua_State *L) {
		if (!_lg_typecheck_base_GetHandle(L)) {
			luaL_error(L, "luna typecheck failed in HWND wxHtmlHelpFrame::base_GetHandle() const function, expected prototype:\nHWND wxHtmlHelpFrame::base_GetHandle() const\nClass arguments details:\n\n%s",luna_dumpStack(L).c_str());
		}


		wxHtmlHelpFrame* self=Luna< wxObject >::checkSubType< wxHtmlHelpFrame >(L,1);
		if(!self) {
			luaL_error(L, "Invalid object in function call HWND wxHtmlHelpFrame::base_GetHandle() const. Got : '%s'\n%s",typeid(Luna< wxObject >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		HWND stack_lret = self->wxHtmlHelpFrame::GetHandle();
...
luna_RegType LunaTraits< wxHtmlHelpFrame >::methods[] = {
	...
	{"base_GetHandle", &luna_wrapper_wxHtmlHelpFrame::_bind_base_GetHandle},
