...
public:
	...
	inline static bool _lg_typecheck_ctor_overload_2(lua_State *L) {
		...
		if( luatop>2 && (!dynamic_cast< wxGBSpan* >(Luna< wxGBSpan >::check(L,3))) ) return false;
	...
	inline static bool _lg_typecheck_ctor_overload_3(lua_State *L) {
		...
		if( (lua_isnil(L,1)==0 && !Luna<void>::has_uniqueid(L,1,56813631)) ) return false;
		if( (lua_isnil(L,1)==0 && !dynamic_cast< wxSizer* >(Luna< wxObject >::check(L,1)) ) ) return false;
		if( !Luna<void>::has_uniqueid(L,2,3425779) ) return false;
		if( (!dynamic_cast< wxGBPosition* >(Luna< wxGBPosition >::check(L,2))) ) return false;
		if( luatop>2 && !Luna<void>::has_uniqueid(L,3,97553085) ) return false;
		if( luatop>2 && (!dynamic_cast< wxGBSpan* >(Luna< wxGBSpan >::check(L,3))) ) return false;
	...
	inline static bool _lg_typecheck_Intersects_overload_1(lua_State *L) {
		...
		if( (!dynamic_cast< wxGBSizerItem* >(Luna< wxObject >::check(L,2))) ) return false;
		return true;
	}
...
luna_RegType LunaTraits< wxGBSizerItem >::methods[] = {
	...
	{"__eq", &luna_wrapper_wxGBSizerItem::_bind___eq},
