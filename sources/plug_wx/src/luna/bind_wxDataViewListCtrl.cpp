...
public:
	...
	inline static bool _lg_typecheck_AppendColumn_overload_1(lua_State *L) {
		...
		if( (lua_isnil(L,2)==0 && !dynamic_cast< wxDataViewColumn* >(Luna< wxHeaderColumn >::check(L,2)) ) ) return false;
		return true;
	}
	...
	inline static bool _lg_typecheck_InsertColumn_overload_1(lua_State *L) {
		...
		if( (lua_isnil(L,3)==0 && !dynamic_cast< wxDataViewColumn* >(Luna< wxHeaderColumn >::check(L,3)) ) ) return false;
		return true;
	}
