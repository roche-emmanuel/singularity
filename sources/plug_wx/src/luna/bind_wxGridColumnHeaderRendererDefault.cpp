...
public:
	...
	static int _bind_base_DrawLabel(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void wxGridColumnHeaderRendererDefault::base_DrawLabel(const wxGrid &, wxDC &, const wxString &, const wxRect &, int, int, int) const");
		}
	...
	static int _bind_base_DrawBorder(lua_State *L) {
		...
		if(!self) {
			luna_printStack(L);
			luaL_error(L, "Invalid object in function call void wxGridColumnHeaderRendererDefault::base_DrawBorder(const wxGrid &, wxDC &, wxRect &) const");
		}
