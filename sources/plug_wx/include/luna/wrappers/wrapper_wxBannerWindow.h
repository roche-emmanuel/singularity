...
public:
	...
	~wrapper_wxBannerWindow() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxBannerWindow*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxBannerWindow(lua_State* L, lua_Table* dum) 
		: wxBannerWindow(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxBannerWindow*)this);
			_obj.callFunction<void>();
		}
	};
	wrapper_wxBannerWindow(lua_State* L, lua_Table* dum, wxWindow * parent, wxDirection dir = ::wxLEFT) 
		: wxBannerWindow(parent, dir), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxBannerWindow*)this);
			_obj.callFunction<void>();
	...
	wrapper_wxBannerWindow(lua_State* L, lua_Table* dum, wxWindow * parent, int winid, wxDirection dir = ::wxLEFT, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = 0, const wxString & name = wxBannerWindowNameStr) 
		: wxBannerWindow(parent, winid, dir, pos, size, style, name), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxBannerWindow*)this);
			_obj.callFunction<void>();
