...
public:
	...
	~wrapper_wxGridCellEnumEditor() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxGridCellEnumEditor*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxGridCellEnumEditor(lua_State* L, lua_Table* dum, const wxString & choices = wxEmptyString) 
		: wxGridCellEnumEditor(choices), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxGridCellEnumEditor*)this);
			_obj.callFunction<void>();
		}
	};
