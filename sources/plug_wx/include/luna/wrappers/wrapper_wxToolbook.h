...
public:
	...
	wrapper_wxToolbook(lua_State* L, lua_Table* dum) : wxToolbook(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxToolbook(lua_State* L, lua_Table* dum, wxWindow * parent, int id, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = 0, const wxString & name = wxEmptyString) : wxToolbook(parent, id, pos, size, style, name), luna_wrapper_base(L) { register_protected_methods(L); };
