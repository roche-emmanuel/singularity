...
public:
	...
	wrapper_wxNotebook(lua_State* L, lua_Table* dum) : wxNotebook(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxNotebook(lua_State* L, lua_Table* dum, wxWindow * parent, int id, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = 0, const wxString & name = wxNotebookNameStr) : wxNotebook(parent, id, pos, size, style, name), luna_wrapper_base(L) { register_protected_methods(L); };
