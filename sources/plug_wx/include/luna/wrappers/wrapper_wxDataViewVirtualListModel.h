#ifndef _WRAPPERS_WRAPPER_WXDATAVIEWVIRTUALLISTMODEL_H_
#define _WRAPPERS_WRAPPER_WXDATAVIEWVIRTUALLISTMODEL_H_
...
public:
	...
	~wrapper_wxDataViewVirtualListModel() {
		logDEBUG3("Calling delete function for wrapper wxDataViewVirtualListModel");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxDataViewVirtualListModel*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxDataViewVirtualListModel(lua_State* L, lua_Table* dum, unsigned int initial_size = 0) 
		: wxDataViewVirtualListModel(initial_size), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxDataViewVirtualListModel*)this);
			_obj.callFunction<void>();
		}
	};
	...
	void Resort() {
		...
		return wxDataViewVirtualListModel::Resort();
	};
