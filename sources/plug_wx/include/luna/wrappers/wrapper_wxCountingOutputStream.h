...
public:
	...
	~wrapper_wxCountingOutputStream() {
		logDEBUG3("Calling delete function for wrapper wxCountingOutputStream");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxCountingOutputStream*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxCountingOutputStream(lua_State* L, lua_Table* dum) 
		: wxCountingOutputStream(), luna_wrapper_base(L) { 
		register_protected_methods(L); 
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxCountingOutputStream*)this);
			_obj.callFunction<void>();
		}
	};
