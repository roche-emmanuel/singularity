...
public:
	...
	wrapper_wxControl(lua_State* L, lua_Table* dum, wxWindow * parent, int id, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = 0, const wxValidator & validator = wxDefaultValidator, const wxString & name = wxControlNameStr) : wxControl(parent, id, pos, size, style, validator, name), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxControl(lua_State* L, lua_Table* dum) : wxControl(), luna_wrapper_base(L) { register_protected_methods(L); };
