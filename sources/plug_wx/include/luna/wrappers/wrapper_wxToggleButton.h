...
public:
	...
	wrapper_wxToggleButton(lua_State* L, lua_Table* dum) : wxToggleButton(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxToggleButton(lua_State* L, lua_Table* dum, wxWindow * parent, int id, const wxString & label, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = 0, const wxValidator & val = wxDefaultValidator, const wxString & name = wxCheckBoxNameStr) : wxToggleButton(parent, id, label, pos, size, style, val, name), luna_wrapper_base(L) { register_protected_methods(L); };
