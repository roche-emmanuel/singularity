...
public:
	...
	~wrapper_wxDataViewRenderer() {
		logDEBUG3("Calling delete function for wrapper wxDataViewRenderer");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxDataViewRenderer*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxDataViewRenderer(lua_State* L, lua_Table* dum, const wxString & varianttype, wxDataViewCellMode mode = ::wxDATAVIEW_CELL_INERT, int align = -1) 
		: wxDataViewRenderer(varianttype, mode, align), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxDataViewRenderer*)this);
			_obj.callFunction<void>();
		}
	};
