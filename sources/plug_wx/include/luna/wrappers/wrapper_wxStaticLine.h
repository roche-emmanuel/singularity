...
public:
	...
	wrapper_wxStaticLine(lua_State* L, lua_Table* dum) : wxStaticLine(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxStaticLine(lua_State* L, lua_Table* dum, wxWindow * parent, int id = ::wxID_ANY, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = wxLI_HORIZONTAL, const wxString & name = wxStaticLineNameStr) : wxStaticLine(parent, id, pos, size, style, name), luna_wrapper_base(L) { register_protected_methods(L); };
