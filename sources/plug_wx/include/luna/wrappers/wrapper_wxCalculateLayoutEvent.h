...
public:
	...
	~wrapper_wxCalculateLayoutEvent() {
		logDEBUG3("Calling delete function for wrapper wxCalculateLayoutEvent");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxCalculateLayoutEvent*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxCalculateLayoutEvent(lua_State* L, lua_Table* dum, int id = 0) 
		: wxCalculateLayoutEvent(id), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxCalculateLayoutEvent*)this);
			_obj.callFunction<void>();
		}
	};
