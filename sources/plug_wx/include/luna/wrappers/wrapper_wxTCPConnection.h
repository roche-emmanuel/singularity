...
public:
	...
	wrapper_wxTCPConnection(lua_State* L, lua_Table* dum) : wxTCPConnection(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxTCPConnection(lua_State* L, lua_Table* dum, void * buffer, size_t size) : wxTCPConnection(buffer, size), luna_wrapper_base(L) { register_protected_methods(L); };
