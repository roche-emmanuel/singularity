...
public:
	...
	~wrapper_wxPaletteChangedEvent() {
		...
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxPaletteChangedEvent*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxPaletteChangedEvent(lua_State* L, lua_Table* dum, int winid = 0) 
		: wxPaletteChangedEvent(winid), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxPaletteChangedEvent*)this);
			_obj.callFunction<void>();
		}
	};
