...
public:
	...
	~wrapper_wxDocChildFrame() {
		logDEBUG3("Calling delete function for wrapper wxDocChildFrame");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxDocChildFrame*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxDocChildFrame(lua_State* L, lua_Table* dum, wxDocument * doc, wxView * view, wxFrame * parent, int id, const wxString & title, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = wxDEFAULT_FRAME_STYLE, const wxString & name = wxFrameNameStr) 
		: wxDocChildFrame(doc, view, parent, id, title, pos, size, style, name), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxDocChildFrame*)this);
			_obj.callFunction<void>();
		}
	};
