...
class wrapper_wxDataObjectSimple : public wxDataObjectSimple, public luna_wrapper_base {

public:
		

	~wrapper_wxDataObjectSimple() {
		logDEBUG3("Calling delete function for wrapper wxDataObjectSimple");
		if(_obj.pushFunction("delete")) {
			//_obj.pushArg((wxDataObjectSimple*)this); // No this argument or the object will be referenced again!
			_obj.callFunction<void>();
		}
	};
	
	wrapper_wxDataObjectSimple(lua_State* L, lua_Table* dum, const wxDataFormat & format = wxFormatInvalid) 
		: wxDataObjectSimple(format), luna_wrapper_base(L) { 
		register_protected_methods(L);
		if(_obj.pushFunction("buildInstance")) {
			_obj.pushArg((wxDataObjectSimple*)this);
			_obj.callFunction<void>();
		}
	};
