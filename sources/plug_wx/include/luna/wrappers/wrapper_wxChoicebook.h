...
public:
	...
	wrapper_wxChoicebook(lua_State* L, lua_Table* dum) : wxChoicebook(), luna_wrapper_base(L) { register_protected_methods(L); };
	wrapper_wxChoicebook(lua_State* L, lua_Table* dum, wxWindow * parent, int id, const wxPoint & pos = wxDefaultPosition, const wxSize & size = wxDefaultSize, long style = 0, const wxString & name = wxEmptyString) : wxChoicebook(parent, id, pos, size, style, name), luna_wrapper_base(L) { register_protected_methods(L); };
