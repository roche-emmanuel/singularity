...
public:
	...
	static int _bind_length(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::length() const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_squaredLength(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::squaredLength() const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// float sgt::vec3f::distanceTo(const sgt::vec3f & v) const
	static int _bind_distanceTo(lua_State *L) {
		if (!_lg_typecheck_distanceTo(L)) {
			luaL_error(L, "luna typecheck failed in float sgt::vec3f::distanceTo(const sgt::vec3f & v) const function, expected prototype:\nfloat sgt::vec3f::distanceTo(const sgt::vec3f & v) const\nClass arguments details:\narg 1 ID = 76239302\n\n%s",luna_dumpStack(L).c_str());
		}

		const sgt::vec3f* v_ptr=(Luna< sgt::vec3f >::check(L,2));
		if( !v_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg v in sgt::vec3f::distanceTo function");
		}
		const sgt::vec3f & v=*v_ptr;

		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::distanceTo(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// float sgt::vec3f::squaredDistanceTo(const sgt::vec3f & v) const
	static int _bind_squaredDistanceTo(lua_State *L) {
		if (!_lg_typecheck_squaredDistanceTo(L)) {
			luaL_error(L, "luna typecheck failed in float sgt::vec3f::squaredDistanceTo(const sgt::vec3f & v) const function, expected prototype:\nfloat sgt::vec3f::squaredDistanceTo(const sgt::vec3f & v) const\nClass arguments details:\narg 1 ID = 76239302\n\n%s",luna_dumpStack(L).c_str());
		}

		const sgt::vec3f* v_ptr=(Luna< sgt::vec3f >::check(L,2));
		if( !v_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg v in sgt::vec3f::squaredDistanceTo function");
		}
		const sgt::vec3f & v=*v_ptr;

		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::squaredDistanceTo(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_dot(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::dot(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_normalize_overload_1(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec3f::normalize() const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_normalize_overload_2(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec3f::normalize(float) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_cross(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec3f::cross(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_xy(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2f sgt::vec3f::xy() const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_getX(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::x(). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->x;
		lua_pushnumber(L,lret);
	...
	static int _bind_getY(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::y(). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->y;
		lua_pushnumber(L,lret);
	...
	static int _bind_getZ(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec3f::z(). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->z;
		lua_pushnumber(L,lret);
	...
	static int _bind_setX(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec3f::x(float). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->x = value;
	...
	static int _bind_setY(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec3f::y(float). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->y = value;
	...
	static int _bind_setZ(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec3f::z(float). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->z = value;
	...
	static int _bind_op_index(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float & sgt::vec3f::operator[](const int). Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___eq(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool sgt::vec3f::operator==(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___add(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec3f::operator+(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___mul_overload_2(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec3f::operator*(const float) const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___unm(lua_State *L) {
		...
		sgt::vec3f* self=(Luna< sgt::vec3f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec3f::operator-() const. Got : '%s'\n%s",typeid(Luna< sgt::vec3f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
...
luna_RegType LunaTraits< sgt::vec3f >::methods[] = {
	...
	{"distanceTo", &luna_wrapper_sgt_vec3f::_bind_distanceTo},
	{"squaredDistanceTo", &luna_wrapper_sgt_vec3f::_bind_squaredDistanceTo},
