...
public:
	...
	static int _bind_dot_overload_1(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec4f::dot(const sgt::vec4f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_dot_overload_2(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec4f::dot(const sgt::vec3f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->dot(v);
		lua_pushnumber(L,lret);
	...
	static int _bind_xyzw(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec4f::xyzw() const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_xyz(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec3f sgt::vec4f::xyz() const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_xy(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2f sgt::vec4f::xy() const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_getX(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec4f::x(). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->x;
		lua_pushnumber(L,lret);
	...
	static int _bind_getY(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec4f::y(). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->y;
		lua_pushnumber(L,lret);
	...
	static int _bind_getZ(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec4f::z(). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->z;
		lua_pushnumber(L,lret);
	...
	static int _bind_getW(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float sgt::vec4f::w(). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		float lret = self->w;
		lua_pushnumber(L,lret);
	...
	static int _bind_setX(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec4f::x(float). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->x = value;
	...
	static int _bind_setY(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec4f::y(float). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->y = value;
	...
	static int _bind_setZ(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec4f::z(float). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->z = value;
	...
	static int _bind_setW(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec4f::w(float). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->w = value;
	...
	static int _bind_op_index(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call float & sgt::vec4f::operator[](const int). Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___eq(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool sgt::vec4f::operator==(const sgt::vec4f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___add(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec4f sgt::vec4f::operator+(const sgt::vec4f &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___mul_overload_2(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec4f sgt::vec4f::operator*(const float) const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___unm(lua_State *L) {
		...
		sgt::vec4f* self=(Luna< sgt::vec4f >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec4f sgt::vec4f::operator-() const. Got : '%s'\n%s",typeid(Luna< sgt::vec4f >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
