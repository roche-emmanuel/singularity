...
public:
	...
	static int _bind_length(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::length() const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_squaredLength(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::squaredLength() const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// int sgt::vec2i::distanceTo(const sgt::vec2i & v) const
	static int _bind_distanceTo(lua_State *L) {
		if (!_lg_typecheck_distanceTo(L)) {
			luaL_error(L, "luna typecheck failed in int sgt::vec2i::distanceTo(const sgt::vec2i & v) const function, expected prototype:\nint sgt::vec2i::distanceTo(const sgt::vec2i & v) const\nClass arguments details:\narg 1 ID = 76239274\n\n%s",luna_dumpStack(L).c_str());
		}

		const sgt::vec2i* v_ptr=(Luna< sgt::vec2i >::check(L,2));
		if( !v_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg v in sgt::vec2i::distanceTo function");
		}
		const sgt::vec2i & v=*v_ptr;

		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::distanceTo(const sgt::vec2i &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	// int sgt::vec2i::squaredDistanceTo(const sgt::vec2i & v) const
	static int _bind_squaredDistanceTo(lua_State *L) {
		if (!_lg_typecheck_squaredDistanceTo(L)) {
			luaL_error(L, "luna typecheck failed in int sgt::vec2i::squaredDistanceTo(const sgt::vec2i & v) const function, expected prototype:\nint sgt::vec2i::squaredDistanceTo(const sgt::vec2i & v) const\nClass arguments details:\narg 1 ID = 76239274\n\n%s",luna_dumpStack(L).c_str());
		}

		const sgt::vec2i* v_ptr=(Luna< sgt::vec2i >::check(L,2));
		if( !v_ptr ) {
			luaL_error(L, "Dereferencing NULL pointer for arg v in sgt::vec2i::squaredDistanceTo function");
		}
		const sgt::vec2i & v=*v_ptr;

		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::squaredDistanceTo(const sgt::vec2i &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_dot(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::dot(const sgt::vec2i &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		int lret = self->dot(v);
		lua_pushnumber(L,lret);
	...
	static int _bind_normalize_overload_1(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2i sgt::vec2i::normalize() const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_normalize_overload_2(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2i sgt::vec2i::normalize(int) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind_getX(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::x(). Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		int lret = self->x;
		lua_pushnumber(L,lret);
	...
	static int _bind_getY(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int sgt::vec2i::y(). Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		int lret = self->y;
		lua_pushnumber(L,lret);
	...
	static int _bind_setX(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec2i::x(int). Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->x = value;
	...
	static int _bind_setY(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call void sgt::vec2i::y(int). Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
		self->y = value;
	...
	static int _bind_op_index(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call int & sgt::vec2i::operator[](const int). Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___eq(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call bool sgt::vec2i::operator==(const sgt::vec2i &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___add(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2i sgt::vec2i::operator+(const sgt::vec2i &) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___mul_overload_2(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2i sgt::vec2i::operator*(const int) const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
	...
	static int _bind___unm(lua_State *L) {
		...
		sgt::vec2i* self=(Luna< sgt::vec2i >::check(L,1));
		if(!self) {
			luaL_error(L, "Invalid object in function call sgt::vec2i sgt::vec2i::operator-() const. Got : '%s'\n%s",typeid(Luna< sgt::vec2i >::check(L,1)).name(),luna_dumpStack(L).c_str());
		}
...
luna_RegType LunaTraits< sgt::vec2i >::methods[] = {
	...
	{"distanceTo", &luna_wrapper_sgt_vec2i::_bind_distanceTo},
	{"squaredDistanceTo", &luna_wrapper_sgt_vec2i::_bind_squaredDistanceTo},
