define(["log","jquery","backbone","base/Tools"],function(log,$,Backbone,tools) {
	...
	var Class = Backbone.Model.extend({
		...
		setSize: function(ww,hh) {
			this._paper.clear();
			var wsize = tools.getWindowSize()
			ww *= wsize.x/2; // Horizontal tick size
			hh *= wsize.y/2; // vertical tick size
			...
			var str = M(c+ww*0.4,c)+L(c+ww,c)+M(c-ww*0.4,c)+L(c-ww,c)
					 +M(c,c+hh*0.75)+L(c,c+hh)+M(c,c-hh*0.75)+L(c,c-hh)
			...
			for(i=-1;i<=1;++i) {
				if(i!=0) {
					str += M(c-ms*0.25,c+hh*i)+L(c+ms*0.25,c+hh*i)	
				}
			}		 
